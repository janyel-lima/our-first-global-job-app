    ...
    <div class="bg-white dark:bg-slate-900 rounded-3xl max-w-md w-full border border-slate-200 dark:border-slate-800 p-6 sm:p-8 space-y-6 animate-scaleUp">
      ...
      <form @submit.prevent="submit" class="space-y-4 overflow-y-auto max-h-[480px] px-3.5 py-1.5 pr-2.5 scrollbar-thin">
        ...
        <div class="space-y-1.5">
          <label class="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-wider block font-bold">{{ t('profileModal.nameLabel') }}</label>
          <input 
            v-model="profileEditName"
            type="text"
            required
            :placeholder="t('profileModal.placeholderName')"
        ...
        <!-- BIO -->
        <div class="space-y-1.5">
          <label class="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-wider block font-bold">{{ t('profileModal.bioLabel') }}</label>
          <textarea 
        ...
        <div v-if="isInstructor" class="pt-3.5 border-t border-slate-100 dark:border-slate-800 space-y-3.5 select-none">
          ...
          <div v-if="signatureType === 'text'" class="space-y-2 animate-fadeIn">
            <label class="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-wider block">{{ t('profileModal.signatureTextLabel') }}</label>
            <input 
              v-model="signatureText"
              type="text"
              :placeholder="t('profileModal.signatureTextPlaceholder')"
              class="w-full text-xs font-bold px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 dark:text-white focus:outline-hidden focus:ring-2 focus:ring-inset focus:ring-blue-500 focus:border-blue-500"
            />
            ...
            <div class="p-3 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-200/60 dark:border-slate-800/60 text-center space-y-1">
              <span class="text-[9px] font-black text-slate-400 uppercase tracking-widest block">{{ t('profileModal.certPreview') }}</span>
          ...
          <div v-if="signatureType === 'drawn'" class="space-y-2.5 animate-fadeIn">
            <div class="flex items-center justify-between">
              <label class="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-wider block">{{ t('profileModal.drawSignatureLabel') }}</label>
              <button 
                type="button" 
        ...
        <div class="pt-3 border-t border-slate-100 dark:border-slate-800 space-y-3">
          ...
          <!-- NEW EMAIL INPUT -->
          <div class="space-y-1">
            <label class="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase block font-bold">{{ t('profileModal.changeEmailLabel') }}</label>
            <div class="relative">
          ...
          <div class="space-y-1">
            <div class="flex justify-between items-center mb-0.5 select-none">
              <label class="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase block font-bold">{{ t('profileModal.changePasswordLabel') }}</label>
            ...
            <div v-if="profileEditPassword" class="space-y-2.5 animate-fadeIn">
              ...
              <div class="p-3 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-155 dark:border-slate-850/60 space-y-2 select-none text-[10px]">
                <span class="text-[9px] font-black text-slate-450 dark:text-slate-500 uppercase tracking-wider block mb-1">{{ t('profileModal.requiredCriteria') }}</span>
                <div class="grid grid-cols-2 gap-x-2 gap-y-1 text-slate-600 dark:text-slate-400 font-semibold">
                  <div class="flex items-center gap-1.5">
                    ...
                    <span :class="{ 'text-emerald-600 dark:text-emerald-400 font-bold': realMinLength }">{{ t('profileModal.minChars') }}</span>
                  </div>
                  <div class="flex items-center gap-1.5">
                    <Check v-if="realSpecial" class="w-3.5 h-3.5 text-emerald-500 shrink-0" />
                    <span v-else class="w-3.5 h-3.5 text-rose-500 font-extrabold shrink-0 text-center leading-none">✕</span>
                    <span :class="{ 'text-emerald-600 dark:text-emerald-400 font-bold': realSpecial }">{{ t('profileModal.oneSpecial') }}</span>
                  </div>
                  <div class="flex items-center gap-1.5">
                    <Check v-if="realUpper" class="w-3.5 h-3.5 text-emerald-500 shrink-0" />
                    <span v-else class="w-3.5 h-3.5 text-rose-500 font-extrabold shrink-0 text-center leading-none">✕</span>
                    <span :class="{ 'text-emerald-600 dark:text-emerald-400 font-bold': realUpper }">{{ t('profileModal.oneUpper') }}</span>
                  </div>
                  <div class="flex items-center gap-1.5">
                    <Check v-if="realLower" class="w-3.5 h-3.5 text-emerald-500 shrink-0" />
                    <span v-else class="w-3.5 h-3.5 text-rose-500 font-extrabold shrink-0 text-center leading-none">✕</span>
                    <span :class="{ 'text-emerald-600 dark:text-emerald-400 font-bold': realLower }">{{ t('profileModal.oneLower') }}</span>
                  </div>
                  <div class="flex items-center gap-1.5 col-span-2">
                    <Check v-if="realNumber" class="w-3.5 h-3.5 text-emerald-500 shrink-0" />
                    <span v-else class="w-3.5 h-3.5 text-rose-500 font-extrabold shrink-0 text-center leading-none">✕</span>
                    <span :class="{ 'text-emerald-600 dark:text-emerald-400 font-bold': realNumber }">{{ t('profileModal.oneNumber') }}</span>
                  </div>
                </div>
              </div>
              ...
              <div class="space-y-1 pt-1">
                <label class="text-[9px] font-black text-slate-400 dark:text-slate-500 uppercase block font-bold">{{ t('profileModal.confirmPasswordLabel') }}</label>
                <div class="relative">
                ...
                <div v-if="profileConfirmPassword" class="text-[9px] font-black select-none flex items-center gap-1 animate-fadeIn">
                  <span v-if="profileEditPassword === profileConfirmPassword" class="text-emerald-600 dark:text-emerald-400 flex items-center gap-1">
                    <span class="inline-block bg-emerald-100 dark:bg-emerald-950/50 p-0.5 rounded-full">✓</span> {{ t('profileModal.passwordsMatch') }}
                  </span>
                  <span v-else class="text-rose-500 flex items-center gap-1">
                    <span class="inline-block bg-rose-100 dark:bg-rose-950/50 px-1 rounded-full">✕</span> {{ t('profileModal.passwordsMismatch') }}
                  </span>
                </div>
              </div>
            </div>
