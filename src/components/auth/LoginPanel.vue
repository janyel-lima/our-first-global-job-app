...
import { Check, Compass, Eye, EyeOff, FileText, Lock, Mail, Scale, User } from 'lucide-vue-next';
...
<template>
  <div id="login-panel"
    class="max-w-md w-full bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-6 sm:p-8 shadow-xl space-y-6 text-center animate-fadeIn">

    <!-- Top Header -->
    <div class="space-y-3">
      <span
        class="inline-flex p-3 bg-blue-50 dark:bg-blue-950/40 text-blue-600 dark:text-blue-300 rounded-2xl mx-auto border border-transparent dark:border-blue-900/20">
        <Compass id="decor-compass" class="w-7 h-7 animate-pulse text-blue-600 dark:text-blue-300" />
      </span>
      <div class="space-y-1">
        <h1 class="text-2xl sm:text-3xl font-black text-slate-900 dark:text-slate-100 tracking-tight leading-none">
          English Volunteer</h1>
        <p class="text-slate-500 dark:text-slate-400 text-xs max-w-sm mx-auto leading-relaxed font-semibold">
          Plataforma comunitária de mini-cursos de inglês, quizzes interativos e formação de turmas gratuitas baseadas
          em voluntariado pedagógico.
        </p>
      </div>
    </div>
    ...
    <div class="space-y-4">
      ...
      <div
        class="bg-slate-100 dark:bg-slate-950 p-1 rounded-xl grid grid-cols-2 gap-1 border border-transparent dark:border-slate-800">
        <button type="button" @click="activeSubTab = 'login'; localError = ''" :class="[
        ...
        <button type="button" @click="activeSubTab = 'register'; localError = ''" :class="[
          'py-2 text-xs font-black rounded-lg transition-all cursor-pointer',
      ...
      <div v-if="activeSubTab === 'login'" class="space-y-4 text-left animate-fadeIn">
        <form @submit.prevent="handleUserLogin" class="space-y-3 pt-1">
          <div>
            ...
            <div class="relative">
              ...
              <input type="email" required v-model="userEmail" placeholder="nome@email.com"
          ...
          <div>
            ...
            <div class="relative">
              ...
              <input :type="showPassword ? 'text' : 'password'" required v-model="userPassword"
              ...
              <button type="button" @click="showPassword = !showPassword"
                class="absolute right-3.5 top-3 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 focus:outline-hidden cursor-pointer flex items-center justify-center"
          ...
          <div v-if="localError"
            class="text-xs text-rose-600 font-bold bg-rose-50 dark:bg-rose-950/30 p-3 rounded-lg border border-rose-100 dark:border-rose-900/40 animate-fadeIn">
            ⚠️ {{ localError }}
          </div>
        ...
        <div class="relative flex py-2 items-center">
          ...
          <span
            class="flex-shrink mx-4 text-slate-400 dark:text-slate-500 text-[10px] font-bold uppercase tracking-wider">ou
            entre com</span>
          <div class="flex-grow border-t border-slate-100 dark:border-slate-800"></div>
        </div>

        <button id="btn-login-google" @click="handleGoogleLogin" :disabled="isLoading"
        ...
        <form @submit.prevent="handleUserSignUp" class="space-y-3.5 pt-1">
          <div>
            ...
            <div class="relative">
              ...
              <input type="text" required v-model="userName" placeholder="Como quer ser chamado(a)"
                class="w-full text-xs bg-slate-50 dark:bg-slate-950 dark:text-white border border-slate-200 dark:border-slate-850 rounded-xl p-3 pl-10 focus:bg-white dark:focus:bg-slate-900 focus:outline-hidden font-medium text-slate-900" />
            </div>
          </div>
          ...
          <div>
            ...
            <div class="relative">
              <Mail class="absolute left-3.5 top-3 w-4 h-4 text-slate-400" />
              <input type="email" required v-model="userEmail" placeholder="seuemail@exemplo.com"
                class="w-full text-xs bg-slate-50 dark:bg-slate-950 dark:text-white border border-slate-200 dark:border-slate-850 rounded-xl p-3 pl-10 focus:bg-white dark:focus:bg-slate-900 focus:outline-hidden font-medium text-slate-900" />
            </div>
          </div>
          ...
          <div>
            ...
            <div class="relative mb-2">
              <Lock class="absolute left-3.5 top-3 w-4 h-4 text-slate-400" />
              <input :type="showRegisterPassword ? 'text' : 'password'" required v-model="userPassword"
            ...
            <div
              class="p-3 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-155 dark:border-slate-850/60 space-y-2 select-none text-[10.5px]">
              <span
                class="text-[9.5px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-wider block mb-1">Critérios
                de Segurança Obrigatórios:</span>
              <div class="grid grid-cols-2 gap-x-2 gap-y-1 text-slate-600 dark:text-slate-400 font-medium">
                <div class="flex items-center gap-1.5">
                  ...
                  <span :class="{ 'text-emerald-600 dark:text-emerald-400 font-bold': passMinLength }">Mínimo 8
                    caracteres</span>
                </div>
                <div class="flex items-center gap-1.5">
                  ...
                  <span v-else
                    class="w-3.5 h-3.5 text-rose-500 font-extrabold shrink-0 text-center leading-none">✕</span>
                  <span :class="{ 'text-emerald-600 dark:text-emerald-400 font-bold': passHasSpecial }">1 Caractere
                    Especial</span>
                </div>
                <div class="flex items-center gap-1.5">
                  ...
                  <span v-else
                    class="w-3.5 h-3.5 text-rose-500 font-extrabold shrink-0 text-center leading-none">✕</span>
                  <span :class="{ 'text-emerald-600 dark:text-emerald-400 font-bold': passHasUpper }">1 Letra
                    Maiúscula</span>
                </div>
                <div class="flex items-center gap-1.5">
                  ...
                  <span v-else
                    class="w-3.5 h-3.5 text-rose-500 font-extrabold shrink-0 text-center leading-none">✕</span>
                  <span :class="{ 'text-emerald-600 dark:text-emerald-400 font-bold': passHasLower }">1 Letra
                    Minúscula</span>
                </div>
                <div class="flex items-center gap-1.5 col-span-2">
                  ...
                  <span v-else
                    class="w-3.5 h-3.5 text-rose-500 font-extrabold shrink-0 text-center leading-none">✕</span>
                  <span :class="{ 'text-emerald-600 dark:text-emerald-400 font-bold': passHasNumber }">Ao menos 1 Número
                    (0-9)</span>
                </div>
              </div>
            </div>
            ...
            <div class="mt-3.5">
              ...
              <div class="relative">
                <Lock class="absolute left-3.5 top-3 w-4 h-4 text-slate-400" />
                <input :type="showRegisterConfirmPassword ? 'text' : 'password'" required v-model="userConfirmPassword"
              ...
              <div v-if="userPassword && userConfirmPassword"
                class="mt-1.5 text-[10px] font-black select-none flex items-center gap-1.5 animate-fadeIn">
                <span v-if="userPassword === userConfirmPassword"
                  class="text-emerald-600 dark:text-emerald-400 flex items-center gap-1">
                  <span class="inline-block bg-emerald-100 dark:bg-emerald-950/50 p-0.5 rounded-full">✓</span> As senhas
                  coincidem!
                </span>
                <span v-else class="text-rose-500 flex items-center gap-1">
                  <span class="inline-block bg-rose-100 dark:bg-rose-950/50 px-1 rounded-full">✕</span> As senhas não
                  coincidem.
                </span>
              </div>
            </div>
          ...
          <div class="p-3.5 bg-slate-100 border border-slate-200 rounded-xl space-y-2 shadow-xs">
            <div class="flex items-start gap-2.5">
              ...
              <div class="text-[11px] leading-relaxed text-slate-800">
                ...
                Para o voluntariado comunitário, seus dados (nome, e-mail, progresso nos cursos e diálogos de estudo)
                serão armazenados com fins puramente pedagógicos. Nós não vendemos seus contatos nem repassamos dados
                para terceiros. Você pode, a qualquer tempo, deletar seu perfil do sistema definitivamente.
              </div>
            </div>
            <div class="flex justify-between items-center pt-2 border-t border-slate-200">
              <button type="button" @click="openLgpdModal"
          ...
          <!-- Consent Checkbox -->
          <div
            class="flex items-start gap-2.5 select-none bg-slate-100 rounded-xl border border-slate-200 p-3 shadow-xs">
            <input id="chk-lgpd-accept" type="checkbox" v-model="userLgpdAccepted"
            ...
            <label for="chk-lgpd-accept" class="text-[11px] font-bold text-slate-800 leading-relaxed cursor-pointer">
              Declaro que li e concordo com os Termos de Voluntariado Pedagógico e a coleta descrita em acordo com as
              normas da LGPD. *
            </label>
          </div>
          ...
          <button type="submit" :disabled="isLoading || !userLgpdAccepted"
            class="w-full py-3 text-white font-extrabold text-xs sm:text-sm rounded-xl transition-all cursor-pointer flex items-center justify-center gap-2 hover:opacity-90 disabled:opacity-40 disabled:cursor-not-allowed"
            style="background-color: var(--primary-color) !important; color: #ffffff !important;">
            <span v-if="isLoading"
              class="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></span>
    ...
    <!-- Bottom label -->
    <div
      class="pt-2 text-[11px] text-slate-400 dark:text-slate-500 font-semibold select-none border-t border-slate-100 dark:border-slate-800 pt-3">
      English Volunteer · Plataforma de Voluntariado Pedagógico
    </div>

    <!-- LGPD Full Policy Modal Dialog -->
    <Teleport to="body">
      <div v-if="showLgpdModal"
        ...
        <div
          ...
          <div class="flex items-start justify-between border-b border-slate-100 dark:border-slate-800 pb-4">
            <div>
              ...
              <p class="text-xs text-slate-400 dark:text-slate-500 font-bold mt-1.5 uppercase tracking-wider">
                Regulamento Geral de Voluntariado Educativo</p>
            </div>
            <button type="button" @click="showLgpdModal = false"
          ...
          <div
            ...
            <p class="font-bold text-slate-900 dark:text-white text-sm sm:text-base border-l-4 pl-3 py-1"
              :style="{ borderColor: 'var(--primary-color)' }">
              Este regulamento dita as diretrizes de tratamento de dados pessoais no English Volunteer, em conformidade
              com o marco civil da internet e a Lei Geral de Proteção de Dados (LGPD - LEI Nº 13.709/2018):
            </p>
            ...
            <div
              ...
              <div class="space-y-1.5">
                ...
                <p class="pl-3.5 text-slate-600 dark:text-slate-400">Ao se registrar como estudante ou professor
                  colaborador, seus dados pessoais serão utilizados exclusivamente para gerenciar as rotinas das turmas
                  de inglês gratuitas, calcular os índices de quizzes respondidos, facilitar as salas de chats tutoradas
                  e emitir seus respectivos certificados. Não fazemos captação indireta de comportamento.</p>
              </div>
              ...
              <div class="space-y-1.5 pt-2 border-t border-slate-100 dark:border-slate-850/40">
                ...
                <p class="pl-3.5 text-slate-600 dark:text-slate-400">Coletamos apenas as peças mínimas fundamentais de
                  identificação: Nome Completo (para fins de confeecção e segurança jurídica do certificado), Endereço
                  de E-mail (chave de login única de autenticação), Nível Teórico de inglês e dados essenciais de
                  andamento pedagógico (como presenças marcadas).</p>
              </div>
              ...
              <div class="space-y-1.5 pt-2 border-t border-slate-100 dark:border-slate-850/40">
                ...
                <p class="pl-3.5 text-slate-600 dark:text-slate-400">Com respeito à integridade dos voluntários,
                  absolutamente nenhum contato ou informação é partilhada com empresas ou entidades do terceiro setor.
                  Nosso banco de dados é restrito, estruturado em ambiente de nuvem seguro.</p>
              </div>
              ...
              <div class="space-y-1.5 pt-2 border-t border-slate-100 dark:border-slate-850/40">
                ...
                <p class="pl-3.5 text-slate-600 dark:text-slate-400">Em obediência ao Artigo 18 da LGPD, o voluntário
                  poderá a qualquer momento exigir do English Volunteer o livre acesso e a total destruição de seus
                  dados pessoais. Seus dados cadastrais serão deletados de forma definitiva e irreversível em até 72
                  horas úteis. <strong>Salvaguarda Acadêmica:</strong> Visando não romper o progresso pedagógico e
                  certificação de estudantes matriculados, quaisquer matérias didáticas ou minicursos criados pelo
                  voluntário permanecerão sob custódia anônima do sistema/administração.</p>
              </div>
              ...
              <div class="space-y-1.5 pt-2 border-t border-slate-100 dark:border-slate-850/40">
                ...
                <p class="pl-3.5 text-slate-600 dark:text-slate-400">Em acordo com a Lei de Voluntariado nacional,
                  declaramos que a presente atividade ofertada por mentores e usufruída pelos alunos não estabelece
                  qualquer liame de natureza trabalhista ou expectativa de remuneração pecuniária direta ou indireta.
                </p>
              </div>
            </div>
          </div>
