...
<template>
    ...
    <div class="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-2 sm:py-3 flex items-center justify-between">
      ...
      <div class="flex items-center gap-1.5 sm:gap-2.5 cursor-pointer shrink-0 animate-fadeIn"
        ...
        <div class="flex flex-col text-left">
          <span class="font-black text-[10px] sm:text-sm tracking-tight text-slate-900 leading-tight">Our First Global
            Job</span>
          <span
      ...
      <div class="flex items-center gap-1.5 sm:gap-6">
        <nav class="hidden md:flex items-center gap-1 bg-slate-100 p-1 rounded-xl border border-slate-150 shadow-inner">
          ...
          <button v-if="userProfile?.isInstructor || userProfile?.isAdmin" id="nav-tab-instructor"
        ...
        <div
          ...
          <div v-if="userProfile" @click="emit('open-profile')"
            ...
            <div class="hidden sm:flex flex-col text-left leading-tight shrink-0 pl-1.5 pr-0.5">
              <p
                class="text-xs font-extrabold text-slate-800 dark:text-slate-200 max-w-[100px] md:max-w-[150px] truncate leading-none mb-1 flex items-center gap-1">
          ...
          <button id="btn-lang-toggle" type="button" @click="toggleLocale" :title="t('header.selectLang')"
            ...
            <span
              class="hidden sm:inline uppercase font-black text-[10px] tracking-wider text-slate-700 dark:text-slate-300 select-none">{{
                locale === 'pt' ? 'EN' : 'PT' }}</span>
          </button>

          <!-- Theme trigger & settings -->
          <div class="relative shrink-0">
            <button type="button" @click="showThemePanel = !showThemePanel" :title="t('header.themeTitle')" :class="[
              'w-8 h-8 sm:w-9 sm:h-9 lg:w-auto lg:px-3 flex items-center justify-center rounded-xl transition-all border cursor-pointer gap-1.5 hover:bg-slate-50 dark:hover:bg-slate-200 border-slate-200 dark:border-slate-250 shadow-sm hover:shadow active:scale-95 shrink-0',
              showThemePanel
              ...
              <span class="hidden lg:inline text-[10px] font-extrabold"
                :class="showThemePanel ? 'text-white' : 'text-slate-700'">{{ locale === 'pt' ? 'Tema' : 'Theme'
                }}</span>
            </button>

            <!-- Dropdown Config Panel -->
            <div v-if="showThemePanel"
              class="absolute right-0 mt-2.5 w-72 sm:w-80 bg-white dark:bg-slate-100 border border-slate-200 dark:border-slate-250 rounded-2xl shadow-xl p-4 sm:p-5 z-50 text-left space-y-4 animate-fadeIn">
              <div class="flex items-center justify-between border-b border-slate-100 pb-2">
                ...
                <button type="button" @click="showThemePanel = false"
                ...
                <div class="grid grid-cols-6 gap-1.5">
                  <button v-for="color in ['#2563EB', '#4F46E5', '#0891B2', '#059669', '#DC2626']" :key="color"
                    ...
                    <span v-if="primaryColor === color"
                      class="absolute inset-0 flex items-center justify-center text-white text-xs font-black">✓</span>
                  </button>
  ...
  <div
    ...
    <button v-if="userProfile?.isInstructor || userProfile?.isAdmin" @click="handleTabClick('instructor')" :class="[
