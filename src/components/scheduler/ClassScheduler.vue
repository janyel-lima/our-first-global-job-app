...
const handleSaveRecordings = (data: { recordingUrl?: string; materialsUrl?: string; materialsList?: ClassMaterialItem[]; notes?: string }) => {
  ...
  selectedRecordingsClass.value = updated;
};
...
const canEditClass = (cl: ClassTurma | null) => {
  if (!cl) return false;
  if (props.isAdmin) return true;
  if ((props.isInstructor || cl.instructorId === props.currentUserId) && cl.status !== 'completed' && cl.status !== 'cancelled') {
    return true;
  }
  return false;
};
...
<template>
  <div class="space-y-6">
      ...
      <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 shrink-0">
        ...
        <div class="flex items-center gap-1 bg-white dark:bg-slate-900 p-1 rounded-xl border border-gray-250 dark:border-slate-700 h-[40px] min-w-[150px]">
          <button
            ...
            :style="viewType === 'list' ? { backgroundColor: 'var(--primary-color)' } : {}"
          >
          ...
          <button
            ...
            :style="viewType === 'calendar' ? { backgroundColor: 'var(--primary-color)' } : {}"
          >
        ...
        <div class="h-full flex flex-col justify-between min-w-0">
          <div class="min-w-0">
            ...
            <div class="flex flex-wrap items-center justify-between gap-2 mb-3 min-w-0">
              <span :class="[
                'text-[10px] uppercase font-black tracking-wider px-2.5 py-0.5 rounded-full shrink-0 border border-transparent',
                cl.status === 'completed' ? 'bg-emerald-100/90 text-emerald-800 dark:bg-emerald-900/60 dark:text-emerald-200 dark:border-emerald-700/60' :
                cl.status === 'cancelled' ? 'bg-rose-100/90 text-rose-800 dark:bg-rose-900/60 dark:text-rose-200 dark:border-rose-700/60' :
                'bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-200 dark:border-slate-700'
               ]">
          ...
          <div class="mt-4 pt-4 border-t border-gray-100 dark:border-slate-700/80 flex flex-col sm:flex-row sm:items-center justify-between gap-3 min-w-0">
            ...
            <div class="flex items-center gap-1.5 flex-wrap shrink-0">
              ...
              <button
                ...
                class="inline-flex items-center gap-1.5 px-2.5 py-1.5 bg-emerald-100/80 hover:bg-emerald-200 dark:bg-emerald-900/60 dark:hover:bg-emerald-800/80 text-emerald-900 dark:text-emerald-200 border border-emerald-200 dark:border-emerald-700/80 rounded-xl text-xs font-bold transition-all cursor-pointer shadow-2xs shrink-0 whitespace-nowrap"
              ...
              >
                <FolderOpen class="w-3.5 h-3.5 text-emerald-700 dark:text-emerald-300 shrink-0" />
        ...
        <div v-else class="p-6 space-y-4.5 overflow-y-auto max-h-[95vh] sm:max-h-[90vh] scrollbar-thin">
          ...
          <div class="flex justify-between items-center pr-8">
            ...
            <div class="flex items-center gap-1.5">
              <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-slate-100 dark:bg-slate-900 text-slate-800 dark:text-slate-100 border border-slate-200/80 dark:border-slate-700/80 font-mono text-xs font-bold shadow-2xs">
                <CalendarDays class="w-3.5 h-3.5 text-blue-600 dark:text-blue-400 shrink-0" />
                <span>{{ formatDisplayDate(activeSelectedClass.scheduledAt) }}</span>
              </span>
              <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-blue-50 dark:bg-blue-950/60 text-blue-900 dark:text-blue-200 border border-blue-200 dark:border-blue-800/80 font-mono text-xs font-extrabold shadow-2xs">
                <Clock class="w-3.5 h-3.5 text-blue-600 dark:text-blue-400 shrink-0" />
                <span>{{ formatDisplayTime(activeSelectedClass.scheduledAt) }}</span>
              </span>
            </div>
          </div>
    ...
    <!-- Completed Class Recordings & Materials Modal -->
    <CompletedClassRecordingsModal
      v-if="selectedRecordingsClass"
      :classItem="selectedRecordingsClass"
      :canManage="isInstructor || isAdmin"
      @close="selectedRecordingsClass = null"
      @save-recordings="handleSaveRecordings"
    />
  </div>
</template>
