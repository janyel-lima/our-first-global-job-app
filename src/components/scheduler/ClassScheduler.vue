            ...
            <div class="flex items-center gap-1">
              ...
              <template v-if="cl.instructorId !== currentUserId">
                ...
                >
                  ...
                  {{ cl.studentIds.length >= cl.maxStudents ? t('scheduler.fullLabel') : t('scheduler.joinLabel') }}
                </button>
              </template>
        ...
        <div class="bg-white dark:bg-slate-900 rounded-2xl border border-gray-150 dark:border-slate-800/80 p-4 shadow-3xs space-y-3.5">
          ...
          >
            ...
            <p class="text-[10px]" :class="cl.status === 'scheduled' ? 'text-white/80' : 'text-gray-450 dark:text-gray-400'">
              {{ t('scheduler.instructorLabel') }} <strong class="font-bold" :class="cl.status === 'scheduled' ? 'text-white' : 'text-gray-600 dark:text-gray-300'">{{ cl.instructorName }}</strong>
            </p>
            <div class="mt-2 pt-2 flex items-center justify-between text-[10px] font-semibold" :class="cl.status === 'scheduled' ? 'border-white/15 text-white/80' : 'border-gray-150/60 dark:border-slate-850 text-gray-400'">
              ...
              <span v-if="cl.studentIds.includes(currentUserId)" class="text-white font-extrabold flex items-center gap-0.5">★ {{ t('scheduler.enrolledLabel') }}</span>
              <span v-else class="text-white font-extrabold">{{ t('scheduler.viewDetails') }}</span>
            </div>
          </div>
        </div>
      </div>
            ...
            <div class="flex items-center gap-1.5">
              ...
              <template v-if="activeSelectedClass.instructorId !== currentUserId">
                ...
                >
                  ...
                  {{ activeSelectedClass.studentIds.length >= activeSelectedClass.maxStudents ? t('scheduler.fullLabel') : t('scheduler.joinLabel') }}
                </button>
              </template>
              <span 
