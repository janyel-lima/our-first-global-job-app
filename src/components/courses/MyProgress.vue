...
import { Award, BookOpen, Check, ChevronLeft, ChevronRight, Search, SlidersHorizontal, User } from 'lucide-vue-next';
...
<template>
  <div id="progress-tracking-panel" class="space-y-6 text-left animate-fadeIn">
    <div
      class="bg-gradient-to-r from-blue-700 to-indigo-800 p-6 sm:p-8 rounded-3xl text-white shadow-lg relative overflow-hidden select-none">
      ...
      <div class="relative z-10 max-w-2xl space-y-2">
        <span
          class="bg-indigo-600/60 border border-indigo-400/30 text-indigo-100 text-[10px] font-black uppercase tracking-widest px-2.5 py-1 rounded-full w-max block">
          Histórico Acadêmico
        </span>
        <h2 class="text-xl sm:text-3xl font-black tracking-tight leading-tight">
          Meu Progresso de Conquistas
        </h2>
        <p class="text-xs sm:text-sm text-indigo-100 leading-relaxed font-semibold">
          Monitore todos os mini-cursos voluntários de inglês que você já concluiu com sucesso na plataforma, emita seus
          certificados comunitários e acompanhe sua evolução de fluência.
        </p>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
      <div
        class="bg-white dark:bg-slate-900 border border-slate-200/75 dark:border-slate-800 p-5 rounded-2xl flex items-center gap-4 shadow-2xs">
        <div
          class="w-12 h-12 rounded-xl bg-orange-50 dark:bg-orange-950/25 text-orange-600 dark:text-orange-400 flex items-center justify-center shrink-0">
          <Award class="w-6 h-6" />
        </div>
        <div>
          <span
            class="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-wider block leading-none mb-1">Cursos
            Concluídos</span>
      ...
      <div
        class="bg-white dark:bg-slate-900 border border-slate-200/75 dark:border-slate-800 p-5 rounded-2xl flex items-center gap-4 shadow-2xs">
        <div
          class="w-12 h-12 rounded-xl bg-blue-50 dark:bg-blue-950/25 text-blue-600 dark:text-blue-400 flex items-center justify-center shrink-0">
          <BookOpen class="w-6 h-6" />
        </div>
        <div>
          <span
            class="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-wider block leading-none mb-1">Lições
            Completadas</span>
          <p class="text-2xl font-black text-slate-800 dark:text-white leading-none">
            {{progressList.filter(p => p.userId === (currentUser?.uid || 'demo-student-uid') && courses.some(c => c.id
              === p.courseId)).reduce((sum, current) => sum + (current.completedLessons?.length || 0), 0) }}
          </p>
        </div>
      </div>
      ...
      <div
        class="bg-white dark:bg-slate-900 border border-slate-200/75 dark:border-slate-800 p-5 rounded-2xl flex items-center gap-4 shadow-2xs">
        <div
          class="w-12 h-12 rounded-xl bg-emerald-50 dark:bg-emerald-950/25 text-emerald-600 dark:text-emerald-400 flex items-center justify-center shrink-0">
          <User class="w-6 h-6" />
        </div>
        <div class="flex-1">
          <span
            class="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-wider block leading-none mb-1">Fluência
            Auto-Avaliada</span>
    ...
    <div class="bg-white dark:bg-slate-900 border border-slate-200/70 dark:border-slate-800 rounded-3xl p-6 space-y-4">
      <div
        class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 dark:border-slate-800/80 pb-4 select-none">
        <div>
          <h3 class="text-sm font-black text-slate-900 dark:text-white uppercase tracking-wider block">Lista Global de
            Certificados Ganhos</h3>
          <p class="text-xs text-slate-400 dark:text-slate-350 font-bold block">Todos os cursos finalizados aparecem
            listados abaixo independente do seu nível de inglês configurado.</p>
        </div>
      </div>

      <!-- Search & Filters Bar -->
      <div v-if="completedCoursesWithCertificates.length > 0"
        class="grid grid-cols-1 sm:grid-cols-12 gap-3 pb-2 select-none">
        <!-- Search Field -->
        <div class="sm:col-span-7 relative">
          ...
          <input id="certificate-search" v-model="searchQuery" type="text"
            placeholder="Pesquisar certificado por título ou descrição..."
        ...
        <div class="sm:col-span-5 relative">
          ...
          <select id="certificate-level-filter" v-model="selectedLevel"
            ...
            <option value="Beginner">Nível Beginner (Iniciante)</option>
            <option value="Intermediate">Nível Intermediate (Intermediário)</option>
            <option value="Advanced">Nível Advanced (Avançado)</option>
          </select>
      ...
      <div v-if="completedCoursesWithCertificates.length === 0"
        class="p-12 text-center bg-slate-50 dark:bg-slate-900/40 border border-slate-200 dark:border-slate-800 rounded-2xl max-w-md mx-auto space-y-3">
        <div
          class="w-12 h-12 bg-slate-100 dark:bg-slate-800 rounded-full flex items-center justify-center mx-auto text-slate-400 dark:text-slate-500">
          <Award class="w-6 h-6" />
        </div>
        <p class="text-xs font-bold text-slate-600 dark:text-slate-350 leading-normal block">
          Você ainda não obteve nenhuma certificação. Complete todos os requisitos (leituras, vídeos ou questionários)
          de qualquer mini-curso voluntário para gerar seu diploma oficial!
        </p>
      ...
      <div v-else-if="filteredCertificates.length === 0"
        class="p-12 text-center bg-slate-50/50 dark:bg-slate-900/40 border border-slate-200 dark:border-slate-800 rounded-2xl max-w-md mx-auto space-y-3 select-none">
        <div
          class="w-12 h-12 bg-slate-100 dark:bg-slate-800 rounded-full flex items-center justify-center mx-auto text-slate-400 dark:text-slate-500">
          <Search class="w-6 h-6" />
        </div>
        <p class="text-xs font-bold text-slate-600 dark:text-slate-350 leading-normal block">
          Nenhum certificado corresponde aos termos pesquisados ou filtros selecionados. Tente alterar sua busca!
        </p>
        <button @click="searchQuery = ''; selectedLevel = 'All';"
          class="px-4 py-1.5 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 font-bold text-xs rounded-lg border border-slate-200 dark:border-slate-750 transition-colors cursor-pointer">
          Limpar Filtros
        </button>
      </div>
      ...
      <div v-else class="space-y-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div v-for="item in paginatedCertificates" :key="item.progress.id"
            ...
            <div class="space-y-1.5">
              <div class="flex items-center justify-between">
                <span class="text-[9px] font-black uppercase tracking-wider px-2 py-0.5 rounded" :class="[
                  ...
                  Nível {{ item.course.level }}
                </span>
                <span class="text-[9px] text-emerald-600 font-black flex items-center gap-0.5">
                  <Check class="w-3 h-3 text-emerald-600" /> Aprovado
                </span>
              </div>
              <h4 class="text-xs sm:text-sm font-black text-slate-800 dark:text-white leading-snug">{{ item.course.title
                }}</h4>
            ...
            <div class="flex items-center justify-between border-t border-slate-200/60 dark:border-slate-800/60 pt-3">
              ...
              <button type="button" @click="$emit('show-certificate', {
        ...
        <div v-if="totalPages > 1"
          class="flex flex-col sm:flex-row items-center justify-between gap-4 border-t border-slate-100 dark:border-slate-800/80 pt-4 select-none">
          ...
          <div class="flex items-center gap-1.5">
            <button id="cert-pagination-prev" @click="prevPage" :disabled="currentPage === 1"
            ...
            <button v-for="p in totalPages" :key="p" id="cert-pagination-page" @click="setPage(p)" :class="[
              p === currentPage
            ...
            <button id="cert-pagination-next" @click="nextPage" :disabled="currentPage === totalPages"
              class="p-2 bg-slate-50 hover:bg-slate-100 dark:bg-slate-950 dark:hover:bg-slate-900 border border-slate-200 dark:border-slate-800 disabled:opacity-40 disabled:hover:bg-slate-50 dark:disabled:hover:bg-slate-950 text-slate-600 dark:text-slate-300 rounded-xl transition cursor-pointer disabled:cursor-not-allowed">
