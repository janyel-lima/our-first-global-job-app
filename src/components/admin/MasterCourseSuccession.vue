...
<template>
  <div>
    ...
    <div class="bg-white dark:bg-slate-900 rounded-3xl border border-gray-100 dark:border-slate-800 p-6 shadow-xs text-left space-y-4">
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-gray-100 dark:border-slate-800 pb-4">
        <div>
          ...
          <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
            Transfira direitos autorais and atribua tutores válidos para cursos de professores desligados da plataforma (solicitação de eliminação de dados pessoais / esquecimento).
          </p>
        </div>
        <div 
          :style="{ 
      ...
      <div class="rounded-xl border border-gray-200/50 dark:border-slate-850 bg-white dark:bg-slate-900 overflow-hidden">
      ...
      <div class="block md:hidden p-4 space-y-3 bg-white dark:bg-slate-900 border-b border-gray-100 dark:border-slate-850">
        <p v-if="filteredDetailedCourses.length === 0" class="text-center text-slate-450 dark:text-slate-400 italic py-6">
          Nenhum curso correspondente aos termos de busca.
        </p>
        <div 
          v-else
        ...
        >
          ...
          <div class="grid grid-cols-2 gap-3 pt-2 border-t border-gray-250/20 dark:border-slate-800/40 text-[11px]">
            <div>
              <span class="block text-[9px] uppercase font-bold text-gray-400 dark:text-gray-500 mb-0.5">Proprietário / Mentor</span>
          ...
          <div class="pt-2 border-t border-gray-250/20 dark:border-slate-800/40 space-y-1">
            <label class="block text-[9px] uppercase font-bold text-gray-400 dark:text-gray-500">Transferir Sucessão</label>
            <select
              :value="course.creatorId"
              @change="(e) => handleReassignCourseOwner(e, course.id, (e.target as HTMLSelectElement).value)"
