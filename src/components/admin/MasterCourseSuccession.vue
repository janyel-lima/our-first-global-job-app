  ...
  <div>
    ...
    <div
      ...
      <div :style="{
        ...
        ecossistema. Isso previne o rompimento do progresso pedagógico de centenas de alunos matriculados e em processo
        de certificação.
      </div>
      ...
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mt-2 select-none">
        <p class="text-[10px] font-black uppercase text-slate-400 dark:text-slate-400 tracking-wider">Histórico de Obras
          Catalogadas</p>
        <div class="relative w-full sm:max-w-xs">
          ...
          <input type="text" placeholder="Pesquisar por curso, nível ou proprietário..." v-model="courseSearchQuery"
      ...
      <div
        ...
        <div v-if="filteredDetailedCourses.length > 0"
          ...
          <span>
            Mostrando <strong class="text-slate-900 dark:text-slate-200">{{ Math.min(filteredDetailedCourses.length,
              (coursesPage - 1) * coursesPerPage + 1) }}</strong> a
            <strong class="text-slate-900 dark:text-slate-200">{{ Math.min(filteredDetailedCourses.length, coursesPage *
              coursesPerPage) }}</strong> de
            <strong class="text-slate-900 dark:text-slate-200">{{ filteredDetailedCourses.length }}</strong> obras
            catalogadas
          </span>
          ...
          <div class="flex items-center gap-1.5">
            ...
            <button type="button" :disabled="coursesPage === totalCoursesPages" @click="coursesPage++"
              class="p-1 px-2.5 bg-slate-50 hover:bg-slate-100 disabled:bg-slate-100 dark:bg-slate-950 dark:hover:bg-slate-850 dark:disabled:bg-slate-950 border border-gray-102 dark:border-slate-850 rounded-lg cursor-pointer disabled:cursor-not-allowed disabled:opacity-40 transition duration-150 inline-flex items-center gap-1 text-[11px]">
              Próxima
    ...
    <div v-if="showReassignDialog && reassignPayload"
      class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fadeIn">
      <div
        class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-850 rounded-2xl shadow-2xl max-w-sm w-full p-5 space-y-4 text-left animate-scaleIn">
        <div class="flex items-center gap-3 border-b border-amber-100 dark:border-slate-800 pb-3">
          <div
            class="p-2 bg-amber-50 dark:bg-amber-950/40 rounded-xl border border-amber-100 dark:border-amber-900/30 text-amber-600">
            <ShieldAlert class="w-5 h-5" />
          </div>
          <div>
            <h3 class="text-sm font-black text-slate-900 dark:text-white uppercase tracking-wider">Sucessão de
              Propriedade</h3>
            <span
              class="text-[9px] bg-amber-100 dark:bg-amber-950/60 border border-amber-200 dark:border-amber-900/30 text-amber-750 dark:text-amber-300 font-extrabold px-1.5 py-0.5 rounded uppercase">LGPD
              / Sucessão</span>
          </div>
        </div>

        <p class="text-[11.5px] text-slate-600 dark:text-slate-300 leading-relaxed font-semibold">
          Deseja realmente transferir de forma permanente todos os direitos autorais, de propriedade intelectual e de
          administração deste curso?
        </p>

        <div
          class="p-3 bg-slate-50 dark:bg-slate-955 border border-slate-100 dark:border-slate-850 rounded-xl space-y-1">
        ...
        <p class="text-[9.5px] text-slate-450 dark:text-slate-400 leading-normal font-medium">
          Isso definirá o novo orientador do curso e atualizará os painéis pedagógicos imediatamente. Esta ação é
          considerada um direito de sucessão definitivo.
        </p>

        <div class="flex gap-2.5 pt-1.5">
          <button type="button" @click="cancelReassignment"
          ...
          <button type="button" @click="confirmReassignment"
