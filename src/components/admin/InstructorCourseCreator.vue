...
<template>
  <div class="space-y-6">
    ...
    <div class="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-5 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-4 text-left shadow-xs">
      <div class="space-y-1">
        <span class="inline-flex items-center gap-1.5 px-3 py-1 bg-blue-50 dark:bg-blue-950/40 text-blue-700 dark:text-blue-300 border border-blue-100 dark:border-blue-900 font-bold text-[10px] rounded-full uppercase tracking-wide">
    ...
    <div v-if="showTutorial" class="bg-slate-50 dark:bg-slate-950 border border-slate-100 dark:border-slate-800 rounded-3xl p-5 text-left space-y-4 shadow-2xs">
      <div>
    ...
    <!-- Import / Export Hub -->
    <div class="bg-slate-50 dark:bg-slate-900 border border-indigo-100/50 dark:border-indigo-900/40 rounded-3xl p-5 text-left flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
      <div class="space-y-1">
    ...
    <div class="bg-white dark:bg-slate-900 p-5 sm:p-6 rounded-3xl border border-slate-200 dark:border-slate-800 text-left space-y-4 shadow-2xs">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div>
          <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Classificação de Nível *</label>
          <select
            required
            v-model="manualLevel"
      ...
      <div>
        <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Sinopse Pedagógica / Descrição *</label>
        <textarea
          required
          rows="2"
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start text-left">
      ...
      <div class="lg:col-span-4 bg-white dark:bg-slate-900 p-5 rounded-3xl border border-slate-200 dark:border-slate-800 space-y-4 shadow-2xs">
        <div class="flex justify-between items-center border-b border-slate-100 dark:border-slate-800 pb-2">
          <span class="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Passo 2: Lições do Programa</span>
          <button
            type="button"
        ...
        <div class="p-3.5 bg-slate-50 dark:bg-indigo-950/50 border border-dashed border-slate-300 dark:border-indigo-500/50 rounded-xl text-center space-y-2 transition-colors">
          <div class="text-center">
            <UploadCloud class="w-6 h-6 text-indigo-600 dark:text-indigo-400 mx-auto" />
            <p class="text-[11px] font-extrabold text-slate-800 dark:text-slate-100 mt-1">Carregar Múltiplos Arquivos .MD</p>
            <p class="text-[10px] font-medium text-slate-500 dark:text-slate-300">Importe textos prontos com tabelas e diálogos para acelerar.</p>
          </div>
          <button
            type="button"
      ...
      <div id="instructor-shield" class="lg:col-span-8 bg-white dark:bg-slate-900 p-5 sm:p-6 rounded-3xl border border-slate-100 dark:border-slate-800 min-h-[400px] shadow-2xs">
        ...
        <div v-else class="space-y-4">
          ...
          <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 dark:border-slate-800 pb-2">
            ...
            <div class="flex items-center gap-2">
              <span class="text-[10px] text-white font-extrabold bg-indigo-600 dark:bg-indigo-600 px-3 py-1 rounded-full uppercase tracking-wider shadow-sm animate-pulse">
                Rascunho Ativo
              </span>
            </div>
          </div>
          ...
          <div v-if="activeLessonTab === 'edit'" class="space-y-4">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Título da Lição *</label>
                <input
                  type="text"
                  required
              ...
              <div class="animate-fadeIn">
                <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1 flex items-center justify-between">
                  <span class="flex items-center gap-1.5">
                    ...
                    <span class="text-[9px] font-bold text-slate-400 dark:text-slate-500 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 px-1.5 py-0.25 rounded uppercase tracking-wider">Opcional</span>
                  </span>
                  ...
                  <span v-else class="text-[9px] font-bold text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 px-1.5 py-0.25 rounded-md">
            ...
            <div class="space-y-1.5 animate-fadeIn">
              <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300">Conteúdo da Lição (Markdown Estilizado) *</label>
              <div class="relative flex flex-col border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden focus-within:ring-2 focus-within:ring-indigo-500/25 focus-within:border-indigo-600 bg-white dark:bg-slate-950">
                <!-- Integrated Toolbar at the top of the editor -->
                <div class="flex flex-row items-center justify-between gap-3 bg-slate-50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 p-2 sm:p-2.5">
                  <div class="flex-1 flex flex-row items-center gap-1 overflow-x-auto py-0.5" style="scrollbar-width: none; -ms-overflow-style: none;">
                    <button
                      type="button"
                    ...
                    >
                      <Bold class="w-3.5 h-3.5 text-blue-500" /> <span class="hidden lg:inline">{{ t('tutor.eslBold') }}</span>
                    </button>
                    <button
                      type="button"
                    ...
                    >
                      <Italic class="w-3.5 h-3.5 text-pink-500" /> <span class="hidden lg:inline">{{ t('tutor.eslItalic') }}</span>
                    </button>
                    ...
                    <button
                      type="button"
                      @click="handleInsertMarkdownSyntax('h1')"
                      ...
                      :title="t('tutor.eslH1')"
                    >
                      <Heading1 class="w-3.5 h-3.5 text-indigo-500" /> <span class="hidden lg:inline">{{ t('tutor.eslH1') }}</span>
                    </button>
                    <button
                      type="button"
                      @click="handleInsertMarkdownSyntax('h2')"
                      class="p-1.5 px-2 bg-transparent hover:bg-slate-200/60 dark:hover:bg-slate-800/80 text-slate-700 dark:text-slate-300 text-[10.5px] font-semibold rounded-lg cursor-pointer transition-all flex items-center gap-1 shrink-0 border-0 active:scale-95 hover:text-indigo-600 dark:hover:text-indigo-400"
                      :title="t('tutor.eslH2')"
                    >
                      <Heading2 class="w-3.5 h-3.5 text-indigo-550" /> <span class="hidden lg:inline">{{ t('tutor.eslH2') }}</span>
                    </button>
                    <button
                      type="button"
                      ...
                      :title="t('tutor.eslH3')"
                    >
                      <Heading3 class="w-3.5 h-3.5 text-indigo-600" /> <span class="hidden lg:inline">{{ t('tutor.eslH3') }}</span>
                    </button>
                    ...
                    >
                      <List class="w-3.5 h-3.5 text-emerald-500" /> <span class="hidden lg:inline">{{ t('tutor.eslList') }}</span>
                    </button>
                    <button
                      type="button"
                      @click="handleInsertMarkdownSyntax('ordered_list')"
                    ...
                    >
                      <ListOrdered class="w-3.5 h-3.5 text-emerald-600" /> <span class="hidden lg:inline">{{ t('tutor.eslOrderedList') }}</span>
                    </button>
                    <button
                      type="button"
                    ...
                    >
                      <CheckSquare class="w-3.5 h-3.5 text-teal-500" /> <span class="hidden lg:inline">{{ t('tutor.eslChecklist') }}</span>
                    </button>
                    ...
                    >
                      <MessageSquare class="w-3.5 h-3.5 text-amber-500" /> <span class="hidden lg:inline">{{ t('tutor.eslDialogue') }}</span>
                    </button>
                    <button
                      type="button"
                    ...
                    >
                      <Table class="w-3.5 h-3.5 text-purple-500" /> <span class="hidden lg:inline">{{ t('tutor.eslGlossary') }}</span>
                    </button>
                    ...
                    >
                      <Lightbulb class="w-3.5 h-3.5 text-amber-500 animate-pulse" /> <span class="hidden lg:inline">{{ t('tutor.eslTipBox') }}</span>
                    </button>
                    <button
                      type="button"
                      @click="handleInsertMarkdownSyntax('warning')"
                    ...
                    >
                      <AlertTriangle class="w-3.5 h-3.5 text-red-500" /> <span class="hidden lg:inline">{{ t('tutor.eslWarningBox') }}</span>
                    </button>
                    <button
                      type="button"
                    ...
                    >
                      <Quote class="w-3.5 h-3.5 text-yellow-500" /> <span class="hidden lg:inline">{{ t('tutor.eslQuote') }}</span>
                    </button>
                    ...
                    >
                      <FileText class="w-3.5 h-3.5 text-rose-500" /> <span class="hidden lg:inline">{{ t('tutor.eslCode') }}</span>
                    </button>
                    <button
                      type="button"
                      @click="handleInsertMarkdownSyntax('codeblock')"
                      class="p-1.5 px-2 bg-transparent hover:bg-slate-200/60 dark:hover:bg-slate-800/80 text-slate-700 dark:text-slate-300 text-[10.5px] font-semibold rounded-lg cursor-pointer transition-all flex items-center gap-1 shrink-0 border-0 active:scale-95 hover:text-rose-600 dark:hover:text-rose-450"
                      :title="t('tutor.eslCodeBlock')"
                    >
                      <Code class="w-3.5 h-3.5 text-rose-600" /> <span class="hidden lg:inline">{{ t('tutor.eslCodeBlock') }}</span>
                    </button>
                    ...
                    >
                      <LinkIcon class="w-3.5 h-3.5 text-sky-500" /> <span class="hidden lg:inline">{{ t('tutor.eslLink') }}</span>
                    </button>
                    <button
                      type="button"
                    ...
                    >
                      <Minus class="w-3.5 h-3.5 text-slate-500" /> <span class="hidden lg:inline">{{ t('tutor.eslDivider') }}</span>
                    </button>
                  </div>
          ...
          <div v-if="activeLessonTab === 'preview'" class="space-y-4">
            <div class="p-3.5 bg-indigo-50 dark:bg-indigo-950/40 rounded-xl border border-indigo-100 dark:border-indigo-900/40 text-xs text-indigo-800 dark:text-indigo-300 leading-relaxed">
    ...
    <div class="bg-slate-50 dark:bg-slate-900 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 text-left shadow-2xs">
      ...
      <div class="flex items-center gap-3">
        <p v-if="isGenerating" class="text-xs text-indigo-600 dark:text-indigo-400 font-bold animate-pulse">Sincronizando com a base...</p>
        <button
          type="button"
    ...
    <Teleport to="body">
      <div v-if="isFullscreen && activeLessonObject" class="fixed inset-0 bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-100 z-[99999] flex flex-col p-4 sm:p-6 overflow-y-auto lg:overflow-hidden select-none animate-fadeIn">
        <!-- Fullscreen Header -->
        <div class="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-4 shrink-0">
          <div class="flex items-center gap-3">
            ...
            <div>
              <div class="flex items-center gap-2">
                <span class="px-2 py-0.5 bg-indigo-50 dark:bg-indigo-950 border border-indigo-100 dark:border-indigo-900/60 text-indigo-700 dark:text-indigo-300 text-[9px] font-black uppercase tracking-wider rounded-md">
              ...
              <h3 class="text-sm font-black text-slate-900 dark:text-white leading-tight mt-1 truncate max-w-sm sm:max-w-md">
        ...
        <div class="flex-1 grid grid-cols-1 lg:grid-cols-2 gap-6 overflow-y-visible lg:overflow-hidden min-h-0 mt-5">
          ...
          <div class="flex flex-col bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 space-y-3 h-[500px] lg:h-full lg:overflow-hidden shrink-0">
            <div class="flex justify-between items-center border-b border-slate-100 dark:border-slate-850 pb-2 shrink-0">
              <span class="text-[10.5px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest flex items-center gap-1.5">
                {{ t('tutor.focusWriting') }}
              </span>
              <span class="text-[9.5px] font-mono text-slate-400 dark:text-slate-500">{{ t('tutor.realtimeFormatting') }}</span>
            </div>

            <!-- Toolbar in Fullscreen -->
            <div class="flex flex-row items-center gap-1 bg-slate-50 dark:bg-slate-955/60 p-2 rounded-xl border border-slate-200 dark:border-slate-800 overflow-x-auto shrink-0" style="scrollbar-width: none; -ms-overflow-style: none;">
              <button
                type="button"
                @click="handleInsertMarkdownSyntax('bold')"
                class="p-1.5 px-2 bg-transparent hover:bg-slate-200/60 dark:hover:bg-slate-800/80 text-slate-700 dark:text-slate-300 text-[10.5px] font-semibold rounded-lg cursor-pointer transition-all flex items-center gap-1 shrink-0 border-0 active:scale-95 hover:text-blue-600 dark:hover:text-blue-400"
                :title="t('tutor.eslBold')"
              >
                <Bold class="w-3.5 h-3.5 text-blue-500" /> <span class="hidden xl:inline">{{ t('tutor.eslBold') }}</span>
              </button>
              <button
                type="button"
                @click="handleInsertMarkdownSyntax('italic')"
                ...
                :title="t('tutor.eslItalic')"
              >
                <Italic class="w-3.5 h-3.5 text-pink-500" /> <span class="hidden xl:inline">{{ t('tutor.eslItalic') }}</span>
              </button>
              ...
              <button
                ...
                :title="t('tutor.eslH1')"
              >
                <Heading1 class="w-3.5 h-3.5 text-indigo-500" /> <span class="hidden xl:inline">{{ t('tutor.eslH1') }}</span>
              </button>
              <button
                type="button"
                @click="handleInsertMarkdownSyntax('h2')"
                ...
                :title="t('tutor.eslH2')"
              >
                <Heading2 class="w-3.5 h-3.5 text-indigo-550" /> <span class="hidden xl:inline">{{ t('tutor.eslH2') }}</span>
              </button>
              <button
                type="button"
                @click="handleInsertMarkdownSyntax('heading')"
                ...
                :title="t('tutor.eslH3')"
              >
                <Heading3 class="w-3.5 h-3.5 text-indigo-600" /> <span class="hidden xl:inline">{{ t('tutor.eslH3') }}</span>
              </button>
              ...
              <button
                ...
                :title="t('tutor.eslList')"
              >
                <List class="w-3.5 h-3.5 text-emerald-500" /> <span class="hidden xl:inline">{{ t('tutor.eslList') }}</span>
              </button>
              <button
                type="button"
                @click="handleInsertMarkdownSyntax('ordered_list')"
                ...
                :title="t('tutor.eslOrderedList')"
              >
                <ListOrdered class="w-3.5 h-3.5 text-emerald-600" /> <span class="hidden xl:inline">{{ t('tutor.eslOrderedList') }}</span>
              </button>
              <button
                type="button"
                @click="handleInsertMarkdownSyntax('task')"
                ...
                :title="t('tutor.eslChecklist')"
              >
                <CheckSquare class="w-3.5 h-3.5 text-teal-500" /> <span class="hidden xl:inline">{{ t('tutor.eslChecklist') }}</span>
              </button>
              ...
              <button
                ...
                :title="t('tutor.eslDialogue')"
              >
                <MessageSquare class="w-3.5 h-3.5 text-amber-500" /> <span class="hidden xl:inline">{{ t('tutor.eslDialogue') }}</span>
              </button>
              <button
                type="button"
                @click="handleInsertMarkdownSyntax('table')"
                ...
                :title="t('tutor.eslGlossary')"
              >
                <Table class="w-3.5 h-3.5 text-purple-500" /> <span class="hidden xl:inline">{{ t('tutor.eslGlossary') }}</span>
              </button>
              ...
              <button
                ...
                :title="t('tutor.eslTipBox')"
              >
                <Lightbulb class="w-3.5 h-3.5 text-amber-500 animate-pulse" /> <span class="hidden xl:inline">{{ t('tutor.eslTipBox') }}</span>
              </button>
              <button
                type="button"
                @click="handleInsertMarkdownSyntax('warning')"
                ...
                :title="t('tutor.eslWarningBox')"
              >
                <AlertTriangle class="w-3.5 h-3.5 text-red-500" /> <span class="hidden xl:inline">{{ t('tutor.eslWarningBox') }}</span>
              </button>
              <button
                type="button"
                @click="handleInsertMarkdownSyntax('quote')"
                ...
                :title="t('tutor.eslQuote')"
              >
                <Quote class="w-3.5 h-3.5 text-yellow-500" /> <span class="hidden xl:inline">{{ t('tutor.eslQuote') }}</span>
              </button>
              ...
              <button
                ...
                :title="t('tutor.eslCode')"
              >
                <FileText class="w-3.5 h-3.5 text-rose-500" /> <span class="hidden xl:inline">{{ t('tutor.eslCode') }}</span>
              </button>
              <button
                type="button"
                @click="handleInsertMarkdownSyntax('codeblock')"
                ...
                :title="t('tutor.eslCodeBlock')"
              >
                <Code class="w-3.5 h-3.5 text-rose-600" /> <span class="hidden xl:inline">{{ t('tutor.eslCodeBlock') }}</span>
              </button>
              ...
              <button
                ...
                :title="t('tutor.eslLink')"
              >
                <LinkIcon class="w-3.5 h-3.5 text-sky-500" /> <span class="hidden xl:inline">{{ t('tutor.eslLink') }}</span>
              </button>
              <button
                type="button"
                @click="handleInsertMarkdownSyntax('rule')"
                ...
                :title="t('tutor.eslDivider')"
              >
                <Minus class="w-3.5 h-3.5 text-slate-500" /> <span class="hidden xl:inline">{{ t('tutor.eslDivider') }}</span>
              </button>
            </div>
            ...
            <div class="flex-1 flex flex-col min-h-0 relative">
              ...
              <div class="bg-slate-50 dark:bg-slate-900 border-t border-slate-150 dark:border-slate-800 p-2.5 px-3 rounded-b-xl flex justify-between items-center text-[10.5px] font-mono text-slate-550">
          ...
          <div class="flex flex-col bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-4 space-y-3 h-[500px] lg:h-full lg:overflow-hidden shrink-0">
            <div class="flex justify-between items-center border-b border-slate-100 dark:border-slate-850 pb-2">
              ...
              <span class="px-2 py-0.5 bg-emerald-50 dark:bg-emerald-950 text-emerald-700 dark:text-emerald-400 border border-emerald-100 dark:border-emerald-900 text-[8.5px] font-black uppercase tracking-wider rounded-md">
                {{ t('tutor.exactRendering') }}
              </span>
            </div>

            <!-- Content preview viewport -->
            <div class="flex-1 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl p-5 overflow-y-auto text-slate-800 dark:text-slate-300">
              <MarkdownRenderer :content="activeLessonObject.content" />
            </div>
          </div>
        </div>
