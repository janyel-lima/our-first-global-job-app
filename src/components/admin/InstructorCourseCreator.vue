...
<template>
  <div class="space-y-6">
    ...
    <!-- Informative banner -->
    <div
      class="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4.5 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-4 text-left shadow-xs">
      <div class="space-y-1">
        <span
          class="inline-flex items-center gap-1 px-2.5 py-0.5 bg-blue-50 dark:bg-blue-950/40 text-blue-700 dark:text-blue-300 border border-blue-100 dark:border-blue-900 font-bold text-[10px] rounded-full uppercase tracking-wide">
    ...
    <!-- Import / Export Hub -->
    <div
      class="bg-slate-50 dark:bg-slate-900 border border-indigo-100/50 dark:border-indigo-900/40 rounded-3xl p-4.5 text-left flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
      <div class="space-y-1">
        <h4
    ...
    <div
      class="bg-white dark:bg-slate-900 p-5 sm:p-6 rounded-3xl border border-slate-100 dark:border-slate-800 text-left space-y-4 shadow-2xs">
      <p
        class="text-xs font-extrabold text-gray-400 dark:text-gray-500 uppercase tracking-widest border-b border-gray-50 dark:border-slate-800 pb-1">
        Passo 1: Metadados do Mini-Curso</p>

      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div class="md:col-span-2">
          <label class="block text-xs font-semibold text-gray-700 dark:text-slate-300 mb-1">Título do Mini-Curso
            *</label>
        ...
        <div>
          <label class="block text-xs font-semibold text-gray-700 dark:text-slate-300 mb-1">Classificação de Nível
            *</label>
      ...
      <div>
        <label class="block text-xs font-semibold text-gray-700 dark:text-slate-300 mb-1">Sinopse Pedagógica / Descrição
          *</label>
      ...
      <div class="border-t border-slate-100 dark:border-slate-800/80 pt-4">
        ...
        <div
          class="grid grid-cols-1 md:grid-cols-3 gap-4 bg-slate-50 dark:bg-slate-950 p-4 rounded-xl border border-slate-200/60 dark:border-slate-850">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start text-left">
      ...
      <div
        class="lg:col-span-4 bg-white dark:bg-slate-900 p-5 rounded-3xl border border-slate-100 dark:border-slate-800 space-y-4 shadow-2xs">
        ...
        <div v-else class="space-y-2">
          <div v-for="(less, idx) in lessonsList" :key="less.tempId" :class="[
            ...
            selectedLessonId === less.tempId
              ? 'bg-blue-50/80 border-blue-400 dark:bg-indigo-950/40 dark:border-indigo-500/80'
      ...
      <div id="instructor-shield"
        ...
        <div v-else class="space-y-4">
          ...
          <div
            class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-gray-150 dark:border-slate-800 pb-2">
            <div
              class="flex bg-slate-100 dark:bg-slate-50 p-0.5 rounded-lg select-none border border-transparent dark:border-slate-100/30">
          ...
          <div v-if="activeLessonTab === 'edit'" class="space-y-4">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label class="block text-xs font-semibold text-gray-700 dark:text-slate-300 mb-1">Título da Lição
                  *</label>
              ...
              <div class="animate-fadeIn">
                <label
                  class="block text-xs font-semibold text-gray-700 dark:text-slate-300 mb-1 flex items-center justify-between">
                  <span class="flex items-center gap-1.5">
                    <span>Vídeo do YouTube</span>
                    <span
                      class="text-[9px] font-bold text-slate-400 dark:text-slate-500 bg-slate-50 dark:bg-slate-100/60 border border-slate-200 dark:border-slate-150 px-1.5 py-0.25 rounded uppercase tracking-wider">Opcional</span>
                  </span>
                  ...
                  <span v-else
                    class="text-[9px] font-bold text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-150 border border-slate-150 dark:border-slate-200 px-1.5 py-0.25 rounded-md">
            ...
            <div class="space-y-1.5 animate-fadeIn">
              <label class="block text-xs font-semibold text-gray-700 dark:text-slate-300">Conteúdo da Lição (Markdown
                Estilizado) *</label>
              <div
                class="relative flex flex-col border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden focus-within:ring-1 focus-within:ring-blue-500 bg-white dark:bg-slate-950">
                ...
                <div
                  ...
                  <div class="flex flex-wrap items-center gap-1">
                    ...
                    <button type="button" @click="handleInsertMarkdownSyntax('heading')"
                      ...
                      :title="t('tutor.eslH3')">
                      <Heading3 class="w-3.5 h-3.5 text-indigo-500" /> <span class="hidden lg:inline">{{
                        t('tutor.eslH3') }}</span>
                    </button>
                    ...
                    <button type="button" @click="handleInsertMarkdownSyntax('alert')"
                      ...
                      <Lightbulb class="w-3.5 h-3.5 text-amber-550 animate-pulse" /> <span class="hidden lg:inline">{{
                    ...
                    <button type="button" @click="handleInsertMarkdownSyntax('rule')"
                      class="p-1.5 px-2 bg-white dark:bg-slate-950 hover:bg-slate-100 dark:hover:bg-slate-850 text-slate-700 dark:text-slate-300 text-[10px] font-bold border border-slate-200 dark:border-slate-855 rounded-md cursor-pointer transition-colors flex items-center gap-1 hover:text-slate-900 dark:hover:text-white"
          ...
          <div v-if="activeLessonTab === 'preview'" class="space-y-4">
            <div
              class="p-3 bg-slate-50 dark:bg-slate-100/30 rounded-xl border border-indigo-100/40 dark:border-indigo-200/20 text-[11px] text-slate-700 dark:text-slate-800 leading-relaxed">
                  ...
                  <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-1">
                    <div v-for="(opt, optIdx) in q.options" :key="optIdx" :class="[
                      ...
                      <span v-if="q.correctAnswer === optIdx"
                        class="ml-1 text-[9px] uppercase bg-emerald-100 dark:bg-emerald-950 text-emerald-800 dark:text-emerald-405 px-1.5 py-0.5 rounded-sm font-bold border border-emerald-200 dark:border-emerald-900/40">Correta</span>
                    </div>
                  </div>
    ...
    <div
      ...
      <div class="flex items-center gap-3">
        <p v-if="isGenerating" class="text-xs text-blue-600 dark:text-blue-400 font-bold animate-pulse">Sincronizando
          com a base...</p>
    ...
    <Teleport to="body">
      <div v-if="isFullscreen && activeLessonObject"
        ...
        <!-- Fullscreen Header -->
        <div
          class="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-850 pb-4">
          <div class="flex items-center gap-3">
          ...
          <div
            ...
            <div
              ...
              <button type="button" @click="handleInsertMarkdownSyntax('heading')"
                ...
                :title="t('tutor.eslH3')">
                <Heading3 class="w-3.5 h-3.5 text-indigo-500" /> <span class="hidden xl:inline">{{ t('tutor.eslH3')
                  }}</span>
              </button>
              <button type="button" @click="handleInsertMarkdownSyntax('list')"
                class="p-1 px-2 bg-white dark:bg-slate-900 hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 text-[10px] font-bold border border-slate-200 dark:border-slate-800 rounded-md cursor-pointer transition-colors flex items-center gap-1 hover:text-emerald-600 dark:hover:text-emerald-400"
              ...
              <button type="button" @click="handleInsertMarkdownSyntax('alert')"
                ...
                :title="t('tutor.eslTipBox')">
                <Lightbulb class="w-3.5 h-3.5 text-amber-550 animate-pulse" /> <span class="hidden xl:inline">{{
                  t('tutor.eslTipBox') }}</span>
              </button>
              <button type="button" @click="handleInsertMarkdownSyntax('quote')"
                class="p-1 px-2 bg-white dark:bg-slate-900 hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 text-[10px] font-bold border border-slate-200 dark:border-slate-800 rounded-md cursor-pointer transition-colors flex items-center gap-1 hover:text-yellow-600 dark:hover:text-yellow-500"
              ...
              <button type="button" @click="handleInsertMarkdownSyntax('code')"
                class="p-1 px-2 bg-white dark:bg-slate-900 hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 text-[10px] font-bold border border-slate-200 dark:border-slate-800 rounded-md cursor-pointer transition-colors flex items-center gap-1 hover:text-rose-600 dark:hover:text-rose-450"
                :title="t('tutor.eslCode')">
