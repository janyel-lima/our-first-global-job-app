...
const handleInsertMarkdownSyntax = (syntax: string) => {
  ...
  switch (syntax) {
    ...
    case 'italic':
      ...
      break;
    case 'h1':
      insertion = `\n# ${selectedText || 'Título Principal H1'}\n`;
      break;
    case 'h2':
      insertion = `\n## ${selectedText || 'Título Secundário H2'}\n`;
      break;
    case 'heading':
    case 'h3':
    ...
    case 'alert':
      ...
      break;
    case 'warning':
      insertion = `\n> ⚠️ **Attention (Atenção / Cuidado):**\n> ${selectedText || 'Ponto de atenção importante ou exceção da regra.'}\n`;
      break;
    case 'quote':
...
<template>
  <div class="space-y-6">
    ...
    <!-- Informative banner -->
    <div
      class="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-5 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-4 text-left shadow-xs">
      <div class="space-y-1">
        <span
          class="inline-flex items-center gap-1.5 px-3 py-1 bg-blue-50 dark:bg-blue-950/40 text-blue-700 dark:text-blue-300 border border-blue-100 dark:border-blue-900 font-bold text-[10px] rounded-full uppercase tracking-wide">
          <Info class="w-3 h-3" /> Construtor Autônomo de Inglês
        </span>
        <h3 class="text-sm font-extrabold text-slate-900 dark:text-blue-400 leading-tight">Canal de Criação de Cursos &
          Lições</h3>
        <p class="text-xs text-slate-600 dark:text-slate-300 leading-snug">
          Crie novos tópicos e aplique modelos prontos de conversação. O assistente formata e consolida as questões
          avaliativas instantaneamente para os alunos.
        </p>
      </div>

      <button type="button" @click="showTutorial = !showTutorial"
        ...
        <BookOpen class="w-3.5 h-3.5 text-indigo-550 dark:text-indigo-400" />
    ...
    <div v-if="showTutorial"
      class="bg-slate-50 dark:bg-slate-950 border border-slate-100 dark:border-slate-800 rounded-3xl p-5 text-left space-y-4 shadow-2xs">
      <div>
        ...
        <p class="text-xs text-slate-500">Selecione um modelo de conversação e termos de inglês para anexar ou
          substituir na grade curricular de seu curso:</p>
      </div>

      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div v-for="tmpl in eslTemplatesList" :key="tmpl.name"
          class="p-4 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl shadow-sm hover:shadow-md transition-all flex flex-col justify-between">
          <div class="space-y-1.5 mb-4">
            <div class="flex items-center gap-1.5 justify-between">
              <span
                class="px-2 py-0.5 bg-blue-50 dark:bg-blue-950/40 text-blue-700 dark:text-blue-300 text-[9px] font-black uppercase tracking-wider rounded border border-blue-100 dark:border-blue-900">
                Modelo de Aula
              </span>
            ...
            <p class="text-[10.5px] text-slate-600 dark:text-slate-300 leading-relaxed">
              Injeta automaticamente diálogo bilíngue, glossário pronunciado, dicas estruturais e quiz avaliativo
              customizado.
            </p>
          </div>

          <div class="flex flex-col sm:flex-row gap-2 pt-2 border-t border-slate-100 dark:border-slate-800">
            <button type="button" @click="handleLoadLessonTemplate(tmpl.title, tmpl.content)"
              class="flex-1 py-1.5 px-3 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-[10.5px] font-extrabold transition-all text-center flex items-center justify-center gap-1 cursor-pointer hover:shadow-xs">
              <PlusCircle class="w-3.5 h-3.5" />
              Criar Nova Lição
            </button>

            <button type="button" :disabled="!selectedLessonId"
              @click="handleApplyTemplateToCurrentLesson(tmpl.title, tmpl.content)" :class="[
                'flex-1 py-1.5 px-3 rounded-xl text-[10.5px] font-extrabold transition-all text-center flex items-center justify-center gap-1 border cursor-pointer',
                selectedLessonId
                  ...
                  : 'bg-slate-50 dark:bg-slate-900 text-slate-400 dark:text-slate-500 border-slate-200 dark:border-slate-800 opacity-60 cursor-not-allowed'
              ]"
    ...
    <!-- Import / Export Hub -->
    <div
      class="bg-slate-50 dark:bg-slate-900 border border-indigo-100/50 dark:border-indigo-900/40 rounded-3xl p-5 text-left flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
      <div class="space-y-1">
        <h4
          class="text-xs font-bold text-indigo-800 dark:text-indigo-300 uppercase tracking-wider flex items-center gap-1.5">
        ...
        <p class="text-[11px] text-slate-600 dark:text-slate-300 leading-relaxed">
          Tem um curso completo formatado em JSON? Carregue-o para preencher automaticamente toda a grade de lições e
          quizzes, ou baixe um modelo pronto para servir de base.
        </p>
      </div>
      <div class="flex flex-wrap gap-2 shrink-0">
        <button type="button" @click="triggerImportClick"
        ...
        <input ref="importFileInputRef" type="file" accept=".json" @change="handleImportCourseJson" class="hidden" />
        <button type="button" @click="downloadDummyTemplate"
          class="px-3.5 py-2 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 border border-indigo-200 dark:border-indigo-900 text-indigo-700 dark:text-indigo-300 rounded-xl text-xs font-bold transition-all shadow-xs flex items-center gap-1.5 cursor-pointer">
    ...
    <div
      class="bg-white dark:bg-slate-900 p-5 sm:p-6 rounded-3xl border border-slate-200 dark:border-slate-800 text-left space-y-4 shadow-2xs">
      <p
        class="text-xs font-extrabold text-slate-400 dark:text-slate-500 uppercase tracking-widest border-b border-slate-100 dark:border-slate-800 pb-1">
        Passo 1: Metadados do Mini-Curso</p>

      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div class="md:col-span-2">
          <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Título do Mini-Curso
            *</label>
          <input type="text" required v-model="manualTitle" placeholder="Ex: Business English: Reuniões e Negociações"
            class="w-full text-xs sm:text-sm bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl p-2.5 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-600 focus:outline-none text-slate-900 dark:text-slate-100 transition-all" />
        </div>
        <div>
          <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Classificação de Nível
            *</label>
          <select required v-model="manualLevel"
      ...
      <div>
        <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Sinopse Pedagógica /
          Descrição *</label>
        <textarea required rows="2" v-model="manualDescription"
      ...
      <div class="border-t border-slate-100 dark:border-slate-800 pt-4">
        ...
        <p class="text-[11px] text-slate-500 mb-3.5 leading-snug">
          Selecione quais conquistas de aula o aluno precisa finalizar para avançar no progresso do curso e obter a sua
          certificação:
        </p>

        <div
          class="grid grid-cols-1 md:grid-cols-3 gap-4 bg-slate-50 dark:bg-slate-950 p-4 rounded-xl border border-slate-200 dark:border-slate-800">
          <label
            class="flex items-start gap-2.5 cursor-pointer hover:bg-white dark:hover:bg-slate-900 p-2 rounded-lg transition-all">
            <input type="checkbox" v-model="requireReading"
              class="mt-1 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500 cursor-pointer" />
            <div>
          ...
          <label
            class="flex items-start gap-2.5 cursor-pointer hover:bg-white dark:hover:bg-slate-900 p-2 rounded-lg transition-all">
            <input type="checkbox" v-model="requireVideo"
              class="mt-1 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500 cursor-pointer" />
            <div>
              <span class="block text-xs font-bold text-slate-800 dark:text-slate-200">Assistir ao Vídeo</span>
              <span class="block text-[10px] text-slate-500 leading-snug">Exige interagir/clicar no vídeo (se
                houver)</span>
            </div>
          </label>

          <div class="space-y-2 hover:bg-white dark:hover:bg-slate-900 p-2 rounded-lg transition-all">
            <label class="flex items-start gap-2.5 cursor-pointer">
              <input type="checkbox" v-model="requireQuiz"
                class="mt-1 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500 cursor-pointer" />
              <div>
                <span class="block text-xs font-bold text-slate-800 dark:text-slate-200">Resolver Mini-Quiz
                  (Fixação)</span>
            ...
            <div v-if="requireQuiz" class="pl-7 pt-1 animate-fadeIn">
              <label class="block text-[10px] font-semibold text-slate-600 dark:text-slate-400 mb-1">Aproveitamento
                Mínimo Necessário (%)</label>
              <div class="flex items-center gap-2">
                <input type="number" min="1" max="100" v-model="minQuizScore"
                  class="w-20 text-xs bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 border border-slate-200 dark:border-slate-800 rounded-md p-1 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-600 focus:outline-none text-center font-bold transition-all" />
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start text-left">
      ...
      <div
        class="lg:col-span-4 bg-white dark:bg-slate-900 p-5 rounded-3xl border border-slate-200 dark:border-slate-800 space-y-4 shadow-2xs">
        <div class="flex justify-between items-center border-b border-slate-100 dark:border-slate-800 pb-2">
          <span class="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Passo 2: Lições do
            Programa</span>
        ...
        <div v-if="lessonsList.length === 0" class="py-6 text-center text-xs text-slate-400 dark:text-slate-500 italic">
          Nenhuma lição adicionada à matriz ainda.
        </div>
        <div v-else class="space-y-2">
          <div v-for="(less, idx) in lessonsList" :key="less.tempId" :class="[
            ...
            <div class="flex items-center gap-1 shrink-0" @click.stop>
              <button type="button" :disabled="idx === 0" @click="handleMoveLesson(idx, 'up')"
                class="p-1 text-slate-500 dark:text-white hover:text-slate-800 dark:hover:text-indigo-200 disabled:opacity-25 hover:bg-slate-200/50 dark:hover:bg-slate-800 rounded transition-all cursor-pointer">
      ...
      <div id="instructor-shield"
        class="lg:col-span-8 bg-white dark:bg-slate-900 p-5 sm:p-6 rounded-3xl border border-slate-100 dark:border-slate-800 min-h-[400px] shadow-2xs">
        ...
        <div v-else class="space-y-4">
          ...
          <div
            class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 dark:border-slate-800 pb-2">
            <div
              class="flex flex-wrap bg-slate-100 dark:bg-slate-950 p-1 rounded-xl select-none border border-slate-200 dark:border-slate-800 gap-1">
              <button type="button" @click="activeLessonTab = 'edit'" :class="[
                'px-3.5 py-1.5 text-[11px] font-bold rounded-lg transition-all cursor-pointer flex items-center gap-1.5',
                activeLessonTab === 'edit'
                  ? 'bg-indigo-600 text-white shadow-xs'
                  : 'text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200'
              ]">
                📝 <span>Editor e Texto</span>
              </button>
              <button type="button" @click="activeLessonTab = 'preview'" :class="[
                'px-3.5 py-1.5 text-[11px] font-bold rounded-lg transition-all cursor-pointer flex items-center gap-1.5',
                activeLessonTab === 'preview'
                  ? 'bg-indigo-600 text-white shadow-xs'
                  : 'text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200'
              ]">
                👁️ <span>Pré-visualização Real</span>
              </button>
              <button type="button" @click="activeLessonTab = 'quiz'" :class="[
                'px-3.5 py-1.5 text-[11px] font-bold rounded-lg transition-all cursor-pointer flex items-center gap-1.5',
                activeLessonTab === 'quiz'
                  ? 'bg-indigo-600 text-white shadow-xs'
                  : 'text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200'
              ]">
                ✔️ <span>Quiz de Fixação ({{ activeLessonObject.quiz.length }})</span>
              </button>
            </div>

            <div class="flex items-center gap-2">
              <span
                class="text-[10px] text-white font-extrabold bg-indigo-600 dark:bg-indigo-600 px-3 py-1 rounded-full uppercase tracking-wider shadow-sm animate-pulse">
                Rascunho Ativo
              </span>
            </div>
          </div>
          ...
          <div v-if="activeLessonTab === 'edit'" class="space-y-4">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Título da Lição
                  *</label>
                <input type="text" required v-model="activeLessonObject.title"
                  class="w-full text-xs sm:text-sm bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl p-2.5 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-600 focus:outline-none text-slate-900 dark:text-slate-100 transition-all" />
              </div>
              <div class="animate-fadeIn">
                <label
                  class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1 flex items-center justify-between">
                  <span class="flex items-center gap-1.5">
                    <span>Vídeo do YouTube</span>
                    <span
                      class="text-[9px] font-bold text-slate-400 dark:text-slate-500 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 px-1.5 py-0.25 rounded uppercase tracking-wider">Opcional</span>
                  </span>
                  <span v-if="requireVideo"
                    class="text-[9px] font-bold text-emerald-700 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-100 dark:border-emerald-900/40 px-1.5 py-0.25 rounded-md flex items-center gap-1">
                    <span class="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span>
                    Obrigatório para concluir aula
                  </span>
                  <span v-else
                    class="text-[9px] font-bold text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 px-1.5 py-0.25 rounded-md">
                    Complementar (informativo)
                  </span>
                </label>
                <input type="text" v-model="activeLessonObject.videoUrl"
                ...
                <p class="text-[10px] leading-tight text-slate-500 dark:text-slate-400 mt-1 font-medium">
                  Suporta links de vídeo inteiros ou IDs simples (Ex: <span
                    class="font-mono text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 px-1 rounded">Fw0S1ZPr6pE</span>).
                </p>
              </div>
            </div>

            <!-- Content Editor Textarea Area with Integrated Toolbar and Stats Bar -->
            <div class="space-y-1.5 animate-fadeIn">
              <label class="block text-xs font-semibold text-slate-700 dark:text-slate-300">Conteúdo da Lição (Markdown
                Estilizado) *</label>
              <div
                ...
                <textarea :id="`textarea-markdown-editor-${activeLessonObject.tempId}`" rows="15" required
                  v-model="activeLessonObject.content" placeholder="Escreva seu material usando markdown..."
                ...
                <!-- Bottom Status Bar -->
                <div
                  class="bg-slate-50 dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800 p-2.5 px-4 flex justify-between items-center text-[10.5px] font-mono text-slate-550 select-none">
                  <span class="flex items-center gap-1.5">
                    <span class="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span>
                    Salvo localmente
                  </span>
          ...
          <div v-if="activeLessonTab === 'preview'" class="space-y-4">
            <div
              class="p-3.5 bg-indigo-50 dark:bg-indigo-950/40 rounded-xl border border-indigo-100 dark:border-indigo-900/40 text-xs text-indigo-800 dark:text-indigo-300 leading-relaxed">
              📍 <strong>Visão de Consumo Prático:</strong> É exatamente dessa maneira que os estudantes visualizarão o
              material em seus respectivos celulares ou computadores. Revise o layout!
            </div>

            <div class="border border-slate-200 dark:border-slate-800 rounded-2xl p-5 bg-white dark:bg-slate-950">
          ...
          <div v-if="activeLessonTab === 'quiz'" class="space-y-6">
            <div
              ...
              <div class="space-y-3 text-left">
                <div>
                  <label class="block text-xs font-bold text-slate-600 dark:text-slate-300 mb-1">Enunciado da Pergunta
                    *</label>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    ...
                    <input type="text" v-model="newOption1" placeholder="Ex: Como vai você?"
                      class="w-full text-xs bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl p-2.5 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-600 focus:outline-none text-slate-900 dark:text-slate-100 transition-all" />
                  </div>
                  <div>
                    <label class="block text-[11px] font-semibold text-slate-600 dark:text-slate-400 mb-0.5">Alternativa
                      B (Opção 2) *</label>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 items-end">
                  <div class="sm:col-span-1">
                    ...
                    <select v-model="newCorrectIndex"
                      class="w-full text-xs bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl p-2.5 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-600 focus:outline-none cursor-pointer font-bold text-slate-800 dark:text-slate-200 transition-all">
                ...
                <div class="flex justify-end pt-1">
                  <button type="button" @click="handleAddQuizQuestion(activeLessonObject.tempId)"
            ...
            <div class="space-y-4 text-left">
              <p class="text-xs font-extrabold text-slate-400 dark:text-slate-500 uppercase tracking-widest pl-1">
                Questões registradas no rascunho</p>
              <div v-if="activeLessonObject.quiz.length === 0"
                class="py-4 text-center text-xs text-slate-400 dark:text-slate-500 italic">
                  ...
                  <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-1">
                    <div v-for="(opt, optIdx) in q.options" :key="optIdx" :class="[
                      ...
                      <span v-if="q.correctAnswer === optIdx"
                        class="ml-1 text-[9px] uppercase bg-emerald-100 dark:bg-emerald-950 text-emerald-800 dark:text-emerald-400 px-1.5 py-0.5 rounded-sm font-bold border border-emerald-200 dark:border-emerald-900/40">Correta</span>
                    </div>
                  </div>
    ...
    <div
      ...
      <div>
        ...
        <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Certifique-se de que a estrutura esteja impecável
          antes de submeter ao banco.</p>
      </div>

      <div class="flex items-center gap-3">
        <p v-if="isGenerating" class="text-xs text-indigo-600 dark:text-indigo-400 font-bold animate-pulse">
          Sincronizando com a base...</p>
        <button type="button" :disabled="isGenerating || lessonsList.length === 0" @click="handleCreateCourseManual"
          class="px-6 py-3 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white font-bold text-xs sm:text-sm rounded-xl hover:shadow-md transition-all shrink-0 flex items-center justify-center gap-1.5 cursor-pointer">
          <Check class="w-4 h-4" />
    ...
    <Teleport to="body">
      <div class="fixed bottom-6 right-6 z-[9999] flex flex-col gap-4 max-w-sm w-full pointer-events-none px-4 sm:px-0">
        <TransitionGroup enter-active-class="transform transition duration-300 ease-out"
          ...
          <div v-if="generationError" key="gen-error"
            ...
            <div class="flex-grow pr-4">
              <p class="font-bold text-xs text-rose-800 dark:text-rose-400 uppercase tracking-wider mb-1">Ajuste
                Necessário</p>
              <p class="text-[11.5px] text-slate-700 dark:text-slate-300 leading-relaxed font-semibold">{{
                generationError }}</p>
            </div>
            <button type="button" @click="generationError = null"
          ...
          <div v-if="importSuccess" key="import-success"
            ...
            <div class="flex-grow pr-4">
              <p class="font-bold text-xs text-blue-800 dark:text-blue-400 uppercase tracking-wider mb-1">Mini-Curso
                Carregado!</p>
              ...
              <p
                class="text-[10px] font-bold text-amber-600 dark:text-amber-400 flex items-center gap-1.5 leading-tight">
                ⚠️ O salvamento definitivo requer postar o curso no botão de publicação abaixo.
              </p>
            </div>
            <button type="button" @click="importSuccess = false"
              class="text-slate-400 hover:text-slate-600 dark:text-slate-500 dark:hover:text-slate-300 ml-1 shrink-0 p-1 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors cursor-pointer">
              <X class="w-4 h-4" />
            </button>
          </div>
          ...
          <div v-if="generationSuccess" key="gen-success"
        ...
        <div
          ...
          <div class="flex items-center gap-3">
            ...
            <div>
              <div class="flex items-center gap-2">
                <span
                  class="px-2 py-0.5 bg-indigo-50 dark:bg-indigo-950 border border-indigo-100 dark:border-indigo-900/60 text-indigo-700 dark:text-indigo-300 text-[9px] font-black uppercase tracking-wider rounded-md">
              ...
              <h3
                class="text-sm font-black text-slate-900 dark:text-white leading-tight mt-1 truncate max-w-sm sm:max-w-md">
          ...
          <div class="flex flex-wrap items-center gap-2.5">
            <div
              class="flex items-center bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-1 shrink-0">
              <span class="text-xs text-slate-500 dark:text-slate-400 px-2 font-bold select-none">{{
                t('tutor.lessonTitleLabel') }}</span>
              <input type="text" v-model="activeLessonObject.title"
            ...
            <button type="button" @click="isFullscreen = false"
              class="px-4 py-2.5 bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-800 dark:text-white font-bold text-xs rounded-xl transition-all shadow-3xs shrink-0 flex items-center justify-center gap-1.5 cursor-pointer border border-slate-200 dark:border-slate-800 active:scale-95">
        ...
        <div class="flex-1 grid grid-cols-1 lg:grid-cols-2 gap-6 overflow-y-visible lg:overflow-hidden min-h-0 mt-5">
          ...
          <div
            ...
            <div
              ...
              <span class="text-[9.5px] font-mono text-slate-400 dark:text-slate-500">{{ t('tutor.realtimeFormatting')
                }}</span>
            </div>
            ...
            <div class="flex-1 flex flex-col min-h-0 relative">
              <textarea :id="`textarea-markdown-editor-fs-${activeLessonObject.tempId}`"
                ...
                class="w-full flex-1 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-t-xl p-4.5 font-mono text-xs sm:text-sm leading-relaxed text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-1 focus:ring-indigo-500 resize-none overflow-y-auto" />
              <!-- Bottom Status Indicators inside Fullscreen -->
              <div
                class="bg-slate-50 dark:bg-slate-900 border-t border-slate-150 dark:border-slate-800 p-2.5 px-3 rounded-b-xl flex justify-between items-center text-[10.5px] font-mono text-slate-550">
          ...
          <div
            ...
            <div class="flex justify-between items-center border-b border-slate-100 dark:border-slate-850 pb-2">
              ...
              <span
                class="px-2 py-0.5 bg-emerald-50 dark:bg-emerald-950 text-emerald-700 dark:text-emerald-400 border border-emerald-100 dark:border-emerald-900 text-[8.5px] font-black uppercase tracking-wider rounded-md">
            ...
            <div
              class="flex-1 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl p-5 overflow-y-auto text-slate-800 dark:text-slate-300">
              <MarkdownRenderer :content="activeLessonObject.content" />
            </div>
          </div>
        </div>
