...
<template>
  <div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
      ...
      <div class="lg:col-span-8 bg-white dark:bg-slate-900 rounded-3xl border border-gray-100 dark:border-slate-800 p-5 sm:p-6 shadow-xs text-left space-y-4">
                ...
                <td class="p-4 flex items-center gap-2.5">
                  ...
                  <div class="truncate">
                    <p class="font-extrabold text-gray-900 dark:text-white flex items-center gap-1.5 leading-tight">
                      ...
                      <span v-if="user.isAdmin" class="inline-flex text-[8.5px] font-black bg-red-50 text-red-700 px-1.5 rounded-sm border border-red-100 dark:bg-red-955/30 dark:border-red-900">Admin</span>
                    </p>
        ...
        <div class="block md:hidden p-4 space-y-3 bg-white dark:bg-slate-900 border-b border-gray-100 dark:border-slate-850">
          <div 
            ...
            <div class="flex items-center gap-2.5">
              ...
              <div class="truncate">
                <div class="font-extrabold text-gray-900 dark:text-white flex items-center gap-1.5 leading-tight">
                  {{ user.displayName || user.email || t('master.anonymousUser') }}
                  <span v-if="user.isAdmin" class="inline-flex text-[8px] font-black bg-red-50 text-red-700 px-1 rounded border border-red-100 dark:bg-red-955/30 dark:border-red-900">Admin</span>
                </div>
                <span class="block text-[10px] text-gray-405 dark:text-gray-500 font-mono mt-0.5 truncate">UID: {{ user.uid }}</span>
              </div>
            </div>
          ...
          <div v-if="filteredUsers.length > 0" class="p-4 border-t border-gray-100 dark:border-slate-850 flex items-center justify-between gap-4 flex-wrap text-xs font-semibold select-none text-slate-450 dark:text-slate-400">
            ...
            <div class="flex items-center gap-1.5">
              ...
              <button
                ...
                class="p-1 px-2.5 bg-slate-50 hover:bg-slate-100 disabled:bg-slate-100 dark:bg-slate-950 dark:hover:bg-slate-850 dark:disabled:bg-slate-950 border border-gray-102 dark:border-slate-850 rounded-lg cursor-pointer disabled:cursor-not-allowed disabled:opacity-40 transition duration-150 inline-flex items-center gap-1 text-[11px]"
              >
                {{ t('master.next') }} <ChevronRight class="w-3.5 h-3.5" />
              </button>
            </div>
          </div>
        </div>
      ...
      <!-- Invite Codes -->
      <div class="lg:col-span-4 bg-white dark:bg-slate-900 rounded-3xl border border-gray-100 dark:border-slate-800 p-5 sm:p-6 shadow-xs text-left space-y-4">
        <div>
        ...
        <div class="space-y-2 border-t border-gray-50 dark:border-slate-800/80 pt-4">
          ...
          <div class="relative w-full">
            ...
            <input
              type="text"
              :placeholder="t('master.filterCodesPlaceholder')"
          ...
          <div v-else class="space-y-1.5 max-h-[300px] overflow-y-auto pr-1">
            ...
            >
              <div class="truncate">
                <p class="font-extrabold text-slate-900 dark:text-white flex items-center gap-1.5 font-mono">
                  {{ item.code }}
                  <span v-if="item.status === 'valid'" class="text-[8px] bg-emerald-100 text-emerald-800 px-1 py-0.1 rounded font-black font-sans uppercase">{{ t('master.statusValid') }}</span>
                  <span v-else class="text-[8px] bg-gray-200 text-gray-600 px-1 py-0.1 rounded font-black font-sans uppercase">{{ t('master.statusUsed') }}</span>
                </p>
                <p class="text-[9px] text-slate-400 dark:text-slate-500 mt-0.5">{{ t('master.createdAtLabel') }} {{ item.createdAt ? new Date(item.createdAt).toLocaleDateString(locale === 'pt' ? 'pt-BR' : 'en-US') : '-' }}</p>
              </div>
          ...
          <div v-if="filteredTeacherCodes.length > 0" class="pt-3 flex items-center justify-between gap-4 flex-wrap text-xs font-semibold select-none text-slate-450 dark:text-slate-400 border-t border-slate-100 dark:border-slate-800/60 mt-3">
        ...
        <div class="space-y-3.5">
          ...
          <div>
            ...
            <input 
              ...
              type="email"
              disabled
              :placeholder="t('master.notInformed')"
