    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
      ...
      <div
        ...
        <div
          ...
          <div>
            ...
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
              {{ locale === 'pt' ? 'Inspecione voluntários registrados, altere níveis curriculares ou revogue acessos.'
                : 'Inspect registered volunteers, modify curriculum levels, or revoke access.' }}
            </p>
          </div>
          ...
          <div class="flex flex-wrap items-center gap-2 select-none">
            <button type="button" @click="exportTeachersXLSX"
              ...
              <Download class="w-3.5 h-3.5" /> {{ locale === 'pt' ? 'Exportar Planilha (XLSX)' : 'Export Spreadsheet
              (XLSX)' }}
            </button>
        ...
        <div v-if="filteredUsers.length === 0"
          ...
          {{ locale === 'pt' ? 'Nenhum cadastro coincide com a pesquisa de patrulha.' : 'No registrations match the
          query.' }}
        </div>
          ...
          <table class="w-full text-left border-collapse text-xs min-w-[700px]">
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-slate-850">
              <tr v-for="user in paginatedUsers" :key="user.uid"
                class="hover:bg-slate-50/50 dark:hover:bg-slate-950/25 transition">
                  ...
                  <div class="truncate">
                    <p class="font-extrabold text-gray-900 dark:text-white flex items-center gap-1.5 leading-tight">
                      ...
                      <span v-if="user.isAdmin"
                        class="inline-flex text-[8.5px] font-black bg-red-50 text-red-700 px-1.5 rounded-sm border border-red-100 dark:bg-red-950/30 dark:border-red-900">Admin</span>
                    </p>
                    <span
          ...
          <div v-if="filteredUsers.length > 0"
            ...
            <span>
              <template v-if="locale === 'pt'">
                Mostrando <strong class="text-slate-900 dark:text-slate-200">{{ Math.min(filteredUsers.length,
                  (usersPage - 1) * usersPerPage + 1) }}</strong> a
                <strong class="text-slate-900 dark:text-slate-200">{{ Math.min(filteredUsers.length, usersPage *
                  usersPerPage) }}</strong> de
                <strong class="text-slate-900 dark:text-slate-200">{{ filteredUsers.length }}</strong> registros
              </template>
              <template v-else>
                Showing <strong class="text-slate-900 dark:text-slate-200">{{ Math.min(filteredUsers.length, (usersPage
                  - 1) * usersPerPage + 1) }}</strong> to
                <strong class="text-slate-900 dark:text-slate-200">{{ Math.min(filteredUsers.length, usersPage *
                  usersPerPage) }}</strong> of
            ...
            <div class="flex items-center gap-1.5">
              ...
              <span class="px-2 text-[11px]">
                {{ locale === 'pt' ? `Página ${usersPage} de ${totalUsersPages}` : `Page ${usersPage} of
                ${totalUsersPages}` }}
              </span>
      ...
      <div
        ...
        <div>
          ...
          <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">{{ locale === 'pt' ? 'Gere ou remova códigos de
            credenciamento autônomo para instrutores parceiros.' : 'Generate or remove autonomous accreditation codes
          for
            partner instructors.' }}</p>
        </div>
        ...
        <div class="space-y-2 border-t border-gray-50 dark:border-slate-800/80 pt-4">
                ...
                <p class="font-extrabold text-slate-950 dark:text-slate-100 flex items-center gap-1.5 font-mono">
                  ...
                  <span v-if="item.status === 'valid'"
                    class="text-[8px] bg-emerald-100 text-emerald-800 px-1 py-0.1 rounded font-black font-sans uppercase">{{
                      locale === 'pt' ? 'Válido' : 'Valid' }}</span>
                  <span v-else
                    class="text-[8px] bg-gray-200 text-gray-600 px-1 py-0.1 rounded font-black font-sans uppercase">{{
                      locale
                    === 'pt' ? 'Usado' : 'Used' }}</span>
                </p>
                <p class="text-[9px] text-slate-400 dark:text-slate-500 mt-0.5">{{ locale === 'pt' ? 'Criado em:' :
                  'Created
                  at:' }} {{ item.createdAt ? new Date(item.createdAt).toLocaleDateString(locale === 'pt' ? 'pt-BR' :
                  'en-US')
                  : '-' }}</p>
              </div>
          ...
          <div v-if="filteredTeacherCodes.length > 0"
            ...
            <span>
              <template v-if="locale === 'pt'">
                Mostrando <strong class="text-slate-900 dark:text-slate-200">{{ Math.min(filteredTeacherCodes.length,
                  (codesPage - 1) * codesPerPage + 1) }}</strong> a
                <strong class="text-slate-900 dark:text-slate-200">{{ Math.min(filteredTeacherCodes.length, codesPage *
                  codesPerPage) }}</strong> de
                <strong class="text-slate-900 dark:text-slate-200">{{ filteredTeacherCodes.length }}</strong> códigos
              </template>
              <template v-else>
                Showing <strong class="text-slate-900 dark:text-slate-200">{{ Math.min(filteredTeacherCodes.length,
                  (codesPage - 1) * codesPerPage + 1) }}</strong> to
                <strong class="text-slate-900 dark:text-slate-200">{{ Math.min(filteredTeacherCodes.length, codesPage *
                  codesPerPage) }}</strong> of
                <strong class="text-slate-900 dark:text-slate-200">{{ filteredTeacherCodes.length }}</strong> codes
              </template>
            </span>
            ...
            <div class="flex items-center gap-1.5">
              ...
              <span class="px-2 text-[11px]">
                {{ locale === 'pt' ? `Página ${codesPage} de ${totalCodesPages}` : `Page ${codesPage} of
                ${totalCodesPages}`
                }}
              </span>
          ...
          <div class="flex justify-between items-start border-b border-slate-100 dark:border-slate-850 pb-2.5">
            <div>
              ...
              <p class="text-xs text-slate-400 dark:text-slate-500">
                {{ locale === 'pt' ? 'Mude e salve dados de cadastro do voluntário diretamente no Firestore.' : 'Change
                and
                save volunteer registration data directly in Firestore.' }}
              </p>
            </div>
          ...
          <div class="space-y-3.5">
            ...
            <div>
              <label class="block text-[10px] font-black uppercase text-slate-450 tracking-wider mb-1">
                {{ locale === 'pt' ? 'Nome Completo do Aluno/Professor (Não editável)' : 'Student/Teacher Full Name
                  (Non -editable)' }}
              </label>
            ...
            <div>
              ...
              <input v-model="editUserEmail" type="email" disabled
            ...
            <div>
              <label class="block text-[10px] font-black uppercase text-slate-450 tracking-wider mb-1">
                {{ locale === 'pt' ? 'Anotações / Bio Pedagógica (Não editável)' : 'Notes / Pedagogical Bio
                  (Non -editable)' }}
              </label>
            ...
            <div class="border rounded-xl p-3 space-y-1.5 transition text-left" :style="{
              ...
              <p class="text-[9.5px] text-slate-650 dark:text-slate-350 leading-normal font-medium">
                {{ locale === 'pt' ? 'Você pode disparar um link de reset de senha oficial do Firebase diretamente para
                o
                e - mail cadastrado deste usuário para que ele troque a própria senha com total sigilo.' : 'You can
                trigger an
                official Firebase password reset link directly to this user\'s registered email so they can change their
                password with absolute confidentiality.' }}
              </p>
