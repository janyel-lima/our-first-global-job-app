  ...
  <div class="grid grid-cols-1 md:grid-cols-12 gap-8 items-stretch h-[calc(100vh-220px)] min-h-[550px] relative">
    ...
    <div :class="[
      ...
      <div class="space-y-4 flex flex-col flex-grow overflow-hidden">
        ...
        <div class="p-3 bg-slate-50 dark:bg-slate-850 border border-slate-100 dark:border-slate-800 rounded-xl space-y-2 shrink-0">
          <div class="relative">
            <input
        ...
        <div v-if="filteredChatRooms.length > 0" class="flex items-center justify-between gap-2 pt-2 border-t border-slate-100 dark:border-slate-800 shrink-0">
          <p class="text-[10px] font-bold text-gray-400 dark:text-slate-500">
            {{ t('chat.showingTicketPage', { current: chatCurrentPage, total: totalChatPages, filterCount: filteredChatRooms.length }) }}
          </p>
          <div class="flex items-center gap-1.5">
            <button
              id="btn-chat-prev-page"
              type="button"
    ...
    <div :class="[
      ...
      <template v-if="selectedRoomId && activeRoom">
        <!-- Active chat header -->
        <div class="border-b border-slate-100 dark:border-slate-800 pb-3 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shrink-0">
          <div class="flex items-start gap-2.5 min-w-0 flex-1">
            <!-- Sleek back button (WhatsApp/Telegram style) -->
            <button
              id="btn-chat-mobile-back"
              type="button"
              @click="emit('select-room', null)"
            ...
            <div class="min-w-0 flex-1">
              <span class="text-[9px] text-blue-600 dark:text-blue-300 font-extrabold uppercase tracking-widest bg-blue-50 dark:bg-blue-950/40 px-2 py-0.5 rounded block w-fit truncate max-w-full">
              ...
              <div class="flex items-center gap-1.5 text-[10px] text-slate-400 dark:text-slate-400 font-medium mt-0.5">
                ...
                <span v-if="!isInstructor">{{ t('chat.professorLabel') }}{{ getProfessorNameForRoom(activeRoom) }}</span>
          ...
          <button
            v-if="activeRoom.status === 'open' && (isInstructor || activeRoom.studentId === currentUserId)"
            id="btn-resolve-chat"
            @click="emit('resolve-room', activeRoom.id)"
        ...
        >
          ...
          <div v-if="activeMessages.length >= messagesLimit" class="flex justify-center pb-2 border-b border-dashed border-slate-100 dark:border-slate-800 mb-2">
            <button 
              type="button"
          ...
          <template v-else>
            <div v-for="group in groupedMessages" :key="group.dateLabel" class="space-y-4">
              ...
              <div class="flex justify-center my-4 sticky top-1 z-10">
                <span class="bg-gray-100/90 dark:bg-slate-800/95 backdrop-blur-xs text-gray-500 dark:text-slate-400 text-[10px] font-extrabold px-3.5 py-1 rounded-full border border-gray-200/50 dark:border-slate-700/60 shadow-3xs uppercase tracking-wider select-none">
                  {{ group.dateLabel }}
                </span>
              </div>
        ...
        <div class="relative shrink-0">
          ...
          <div v-if="showEmojiPicker" class="absolute bottom-16 right-0 left-0 bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 shadow-2xl rounded-2xl p-4 z-50 animate-fadeIn flex flex-col">
          ...
          <form 
            ...
            id="form-chat-send-message"
            @submit.prevent="handleSend" 
            class="flex gap-2 items-center w-full"
          >
            <!-- Integrated Input Field with Emoji Button inside -->
            <div class="flex-1 flex items-center bg-slate-50 hover:bg-slate-100/30 dark:bg-slate-850 dark:hover:bg-slate-800 border border-gray-200 dark:border-slate-755 rounded-2xl px-2.5 py-1.5 focus-within:ring-2 focus-within:ring-blue-500/20 focus-within:bg-white dark:focus-within:bg-slate-900 transition-all">
