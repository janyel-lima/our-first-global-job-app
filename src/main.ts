...
if (typeof window !== 'undefined') {
  window.addEventListener('unhandledrejection', event => {
    if (
      ...
      console.warn(
        '[Benign Sandbox Error Ignored]: HMR WebSocket has been suppressed as HMR is disabled by the platform.'
      );
    }
  });
      ...
      caches.keys().then(keys => {
        keys.forEach(key => {
          caches.delete(key).then(() => {
            console.log(
              `Cache "${key}" deleted dynamically to prevent stale Vue asset compilation.`
            );
          });
        });
      });
