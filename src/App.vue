...
const isUserAdmin = computed(() => {
  return !!(userProfile.value?.isAdmin || isMasterEnabled.value);
});

const isUserInstructor = computed(() => {
...
const filteredCourses = computed(() => {
  return courses.value.filter(course => {
    const currentUid = currentUser.value?.uid || userProfile.value?.uid;
    if (currentUid && course.creatorId === currentUid) {
      return false;
    }
...
// Active token verification & offline session security policies
const checkOfflineSessionTTL = () => {
  if (isDemoUser.value || !currentUser.value) return;

  const lastVerifyStr = localStorage.getItem("last_token_verification_timestamp");
  if (!lastVerifyStr) {
    localStorage.setItem("last_token_verification_timestamp", String(Date.now()));
    return;
  }

  const lastVerify = Number(lastVerifyStr);
  const now = Date.now();
  const diffMs = now - lastVerify;
  const oneDayMs = 24 * 60 * 60 * 1000;
  const maxOfflineDays = 7;
  const maxOfflineMs = maxOfflineDays * oneDayMs;

  if (diffMs > maxOfflineMs) {
    console.warn("[SECURITY] Offline security lease expired. Revoking local session cache.");
    showToast("Sua sessão offline de segurança expirou (máximo de 7 dias). Por favor, conecte-se à internet para restabelecer sua autenticação.", "warning", 10000);
    handleLogout();
  } else if (diffMs > (maxOfflineDays - 1) * oneDayMs) {
    const hoursLeft = Math.ceil((maxOfflineMs - diffMs) / (60 * 60 * 1000));
    showToast(`Sessão offline segura prestes a expirar. Conecte-se à internet dentro de ${hoursLeft} hora(s) para renovar suas credenciais.`, "warning", 8000);
  } else {
    const daysUsed = (diffMs / oneDayMs).toFixed(1);
    console.log(`[SECURITY] Sessão offline segura ativa. Tempo sem verificação online: ${daysUsed} dia(s).`);
  }
};

const verifyTokenSecurely = async (force = false) => {
  if (isDemoUser.value) return;
  const user = auth.currentUser;
  if (!user) return;

  if (!isOnline.value) {
    checkOfflineSessionTTL();
    return;
  }
  ...
  try {
    ...
    localStorage.setItem("last_token_verification_timestamp", String(Date.now()));
    console.log("[SECURITY] Token de autenticação verificado com sucesso contra o servidor.");
  } catch (err: any) {
    console.warn("[SECURITY] Falha na verificação de autenticidade do token:", err);
    ...
    if (isInvalidUser) {
      console.warn("[SECURITY] Usuário inativo ou revogado no servidor. Efetuando logout imediato para conformidade de segurança.");
      showToast("Sua sessão foi revogada ou bloqueada no servidor. Faça login novamente.", "error", 10000);
      await handleLogout();
    }
  }
};
...
onMounted(() => {
  auth.onAuthStateChanged(async (user) => {
    if (user) {
      ...
      onSnapshot(profileRef, (snap) => {
        ...
        // Se já temos um perfil carregado para este usuário, mantemos o perfil atual para evitar downgrade por erro de rede
        if (userProfile.value && userProfile.value.uid === user.uid) {
          console.log("[AUTH] Mantendo o perfil do usuário atual apesar do erro de snapshot.");
          isOnboarding.value = false;
          return;
        }

        const deservesAdmin = hasAdminClaim || isSimulatedAdmin || isMaster;
        const deservesInstructor = deservesAdmin || isSimulatedInstructor || isMaster;
        userProfile.value = {
          uid: user.uid,
  ...
  if (typeof window !== "undefined") {
    // Initial offline TTL check and server-side token validation
    checkOfflineSessionTTL();
    if (window.navigator.onLine) {
      verifyTokenSecurely(true);
    }

    // Periodic token verification (every 30 minutes) to check for account deactivations
    ...
    window.addEventListener("online", () => {
      isOnline.value = true;
      verifyTokenSecurely(true); // Immediate validation upon reconnection before syncing
      syncOfflineProgress();
      syncOfflineClassActions();
    });
      ...
      <div v-else class="w-full flex-1">
        ...
        <div v-if="activeTab === 'courses'" class="space-y-6">
          <div v-if="!activeCourseId" class="space-y-8 text-left">
            <StudentCourses :courses="courses" :lessons="lessons" :progressList="activeProgressList"
        ...
        <div v-if="activeTab === 'instructor'">
          <InstructorPanel :courses="courses" :progressReports="activeProgressList" :chatRooms="chatRooms"
            :classes="classes" :lessons="lessons" :users="allUsers"
            :userDisplayName="userProfile?.displayName || 'Instrutor'" :instructorId="userProfile?.uid || ''"
            :uploadCourseFn="handleUploadCourse" :deleteCourseFn="handleDeleteCourse"
            @upload-course="handleUploadCourse" @delete-course="handleDeleteCourse"
            @update-course-config="handleUpdateCourseConfig" />
        </div>

        <!-- Tab 5: Master control Panel monitoring -->
        <div v-if="activeTab === 'master'">
          <MasterPanel :users="allUsers" :courses="courses" :classes="classes" :progressReports="activeProgressList"
        ...
        <div v-if="activeTab === 'tracking'" class="space-y-6 text-left">
          <MyProgress :completedCoursesWithCertificates="completedCoursesWithCertificates"
            :progressList="activeProgressList" :courses="courses" :currentUser="currentUser" :userProfile="userProfile"
            :primaryColor="primaryColor" @open-profile="openProfileModal"
            @show-certificate="(cert) => showCertificateData = cert" @navigate-courses="activeTab = 'courses'" />
        </div>
