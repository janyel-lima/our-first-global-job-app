...
<template>
  ...
  <div v-else
      ...
      <div v-else class="w-full flex-1">
        ...
        <div v-if="activeTab === 'scheduler'">
          <ClassScheduler :classes="classes" :courses="courses" :currentUserId="currentUser?.uid || 'demo-student-uid'"
            ...
            :isAdmin="userProfile?.isAdmin || false" :userLevel="userProfile?.level || 'Beginner'"
            :userDisplayName="userProfile?.displayName || currentUser?.displayName || 'Voluntário'"
            @join="handleJoinClass" @leave="handleLeaveClass" @create="handleCreateClass" @delete="handleDeleteClass"
            @join-class="handleJoinClass" @leave-class="handleLeaveClass" @create-class="handleCreateClass"
            @delete-class="handleDeleteClass" @update-class="handleUpdateClass" @mark-presence="handleMarkPresence" />
        </div>
        ...
        <div v-if="activeTab === 'chats'">
          <SupportChats :chatRooms="chatRooms" :activeMessages="activeMessages" :courses="courses"
            ...
            :isInstructor="userProfile?.isInstructor || userProfile?.isAdmin || false"
            :isAdmin="userProfile?.isAdmin || false" :selectedRoomId="selectedRoomId" :messagesLimit="chatMessagesLimit"
            :isDarkMode="isDarkMode" @select-room="(roomId) => selectedRoomId = roomId"
            @start-room="handleStartChatRoom" @send-message="handleSendMessage" @resolve-room="handleResolveRoom"
    ...
    <CertificateViewer v-if="showCertificateData" :studentName="showCertificateData.studentName"
      ...
      :isMaster="isMasterEnabled" @close="showCertificateData = null" />
    ...
    <UserProfileModal :isOpen="showProfileModal" :initialName="profileEditName" :initialLevel="profileEditLevel"
      ...
      :initialSignatureType="userProfile?.signatureType" :initialSignatureText="userProfile?.signatureText"
      :initialSignatureImage="userProfile?.signatureImage" @close="showProfileModal = false"
      @update-profile="onUpdateProfileFromModal" @delete-account="handleRequestLgpdDataDeletion" />
